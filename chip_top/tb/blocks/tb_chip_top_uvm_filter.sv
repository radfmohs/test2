wire [(20-1):0] notch_coeffs [9:0][15:0];        //[65536Hz - 128Hz] [coeff_18 : coeff_0]
...
//##########################  HPF -> LPF -> NOTCH  ##########################
     ...
     // check HPF output = LPF input
     if(`IMEAS_WRAPPER_TOP.genblk1[i].u_filter_wrapper.U_filter_iir_hpf.filter_out != `IMEAS_WRAPPER_TOP.genblk1[i].u_filter_wrapper.u_filter_fir_lpf.filter_in)
      `nnc_error("SOC_TEST",$sformatf("Connection MISMATCH in FILTER_NUM =%0d HPF output=%0h and LPF input= %0h,",i,`IMEAS_WRAPPER_TOP.genblk1[i].u_filter_wrapper.U_filter_iir_hpf.filter_out,`IMEAS_WRAPPER_TOP.genblk1[i].u_filter_wrapper.u_filter_fir_lpf.filter_in));
