...
# NOTE: Keep the SVF OPEN through create_test_protocol / insert_dft AND the
# post-DFT 'compile_ultra -incremental' below. That incremental compile
# restructures logic (e.g. the flattened U_filter_iir_hpf/accum_reg datapath and
# spi_slv_ctrl), and Formality needs its guidance to verify prescan-vs-postscan
# gate-to-gate (Nanochap_imp_fm_dft.tcl); without it those points fail with
# shifted/unmatched cones. The test-mode setup also recorded here is benign for
# the gate-to-gate functional check (test pins are tied to functional values).
# The SVF is closed later, after the netlist is written (set_svf -off below).
