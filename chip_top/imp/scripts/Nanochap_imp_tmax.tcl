#setenv stage postscan_dct
#setenv generate_sdf sdf
#setenv bottom_up yes
exec mkdir -p ../reports/atpg_$env(stage);
exec mkdir -p ../data/atpg_patterns_$env(stage);
...
if {$env(stage) == "postlayout"} {
	...
	read_netlist ../data/synthesis_$env(stage).BUD=$env(bottom_up)_$env(generate_sdf)/Nanochap_ENS2.$env(stage).v
}
