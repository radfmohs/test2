...
//--------------------------------------------------------------------------------------
// File Name	: soc_wavegen_drva_burstpulse_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_wavegen_drva_burstpulse_test                                             
...
//************************************************************************************
// NOTE : This test follows the same configuration sequence as pulse test, 
//        but additionally use wavegen interrupt feature to update silent 
//        time such that a burst of pulse output wave will be generated.
//************************************************************************************

// =================================================
// Testcase name is defined
// -------------------------------------------------
`define TESTNAME soc_wavegen_drva_burstpulse_test
`define TESTCFG soc_wavegen_drva_burstpulse_test_cfg
//7 bursts
...
class `TESTCFG extends soc_wavegen_base_test_cfg;
  ...
  function new (string name = "soc_wavegen_drva_burstpulse_test_cfg");
    super.new(name);
  ...
  // spi_sclk_freq[15:0]
  constraint c_spi_sclk_freq   { solve spi_sclk_jitter before spi_sclk_freq; spi_sclk_freq inside {[5000:(`SPI_MAX_FREQ/100)*(100 - spi_sclk_jitter)]};}//min 5Mhz - max 14Mhz
...
*/
  ...
  //pclk_div[2:0]
  constraint c_pclk_sel        { soft pclk_sel inside {[1:7]};}
...
class `TESTNAME extends soc_wavegen_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_wavegen_drva_burstpulse_test start", NNC_LOW)
    ...
    wavegen_calc_clock_num(top_test_cfg.clk_freq, 200, 1000, top_test_cfg.half_period, top_test_cfg.half_period);
   ...
   join
    ...
    // --------------------------------------------------------
    #10ms;
    `nnc_info("SOC_TEST", "soc_wavegen_drva_burstpulse_test end now", NNC_LOW)
  ...
  begin
	...
	// --------------------------------------------------------
    	...
    	// --------------------------------------------------------
	assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_IN_WAVE_REG01 + WG_BASE); wr_data[0] == 8'hC8;});
	`WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);
    end
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_INT_REG01 + WG_BASE); no_of_bytes == 3; wr_data[0] == 8'h3E; wr_data[1] == 8'h01; wr_data[2] == 8'h01;});
    `nnc_info("SOC_TEST", "Configure interrupt register", NNC_LOW)//interrupt register set as first address 1; second address 62
    `WR_BURST_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.wr_data);
  end
  endtask
	  ...
	  if ((top_test_cfg.rd_data[2][3]===1'b1) & (top_test_cfg.rd_data[2][2:0]===`DUT_IF.wg_drv_sel)) begin //also check if interrupt in this block is enabled (bit 8 is active)
		...
		if(top_test_cfg.rd_data[2][4]===1) begin
			...
			//wavegen_calc_clock_num(clk_freq (MHz), rest_t (us), silent_t (us), hlf_wave_per (us), neg_hlf_wave_per (us))
    			wavegen_calc_clock_num(top_test_cfg.clk_freq, 200, 9000, 400, 400);
			...
			wr_data[0] == `DUT_IF.wg_silent_wave0_lim[`DUT_IF.wg_drv_sel][23:16]; wr_data[1] == `DUT_IF.wg_silent_wave0_lim[`DUT_IF.wg_drv_sel][15:8]; wr_data[2] == `DUT_IF.wg_silent_wave0_lim[`DUT_IF.wg_drv_sel][7:0];});
    			`nnc_info("SOC_TEST", "Set silent period", NNC_LOW)//0x0000_2328 (9000us)
    			`WR_BURST_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.wr_data);
			// --------------------------------------------------------
			...
			`nnc_info("SOC_TEST", "Updating the period ...", NNC_LOW)
			//wavegen_calc_clock_num(clk_freq (MHz), rest_t (us), silent_t (us), hlf_wave_per (us), neg_hlf_wave_per (us))
    			wavegen_calc_clock_num(top_test_cfg.clk_freq, 200, 1000, 400, 400);
			`DUT_IF.wg_hlf_wave0_lim[`DUT_IF.wg_drv_sel] = top_test_cfg.hlf_wave_lim / 64;
    			`DUT_IF.wg_neg_hlf_wave0_lim[`DUT_IF.wg_drv_sel] = top_test_cfg.neg_hlf_wave_lim / 64;
    			`DUT_IF.wg_rest_wave0_lim[`DUT_IF.wg_drv_sel] = top_test_cfg.rest_lim;
    			`DUT_IF.wg_silent_wave0_lim[`DUT_IF.wg_drv_sel] = top_test_cfg.silent_lim;
			...
			// --------------------------------------------------------
   			// Write to ADDR_WG_DRV_IN_WAVE_REG02
    			// --------------------------------------------------------
			assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_INT_NUM_REG + (`WAVEGEN_1_ADDR_BASE * `DUT_IF.wg_drv_sel)); wr_data[0] == 8'h0B;});
    			`WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);
			s = 1;
		    end
		    // ------------------------------------------------------------------
...
/*		    if (top_test_cfg.rd_data[1] === 8'h01) begin //first address is at the first half of the wave; now load the second half
			if (s==0) begin
				if (j==0) begin
					...
					// --------------------------------------------------------
    					...
    					// --------------------------------------------------------
    					assert(top_test_cfg.randomize() with 
					{reg_addr == (`SOC_ADDR_WG_DRV_SILENT_T_REG01 + (`DUT_IF.wg_drv_sel*`WAVEGEN_1_ADDR_BASE)); no_of_bytes == 4; 
					wr_data[0] == `DUT_IF.wg_silent_wave0_lim[`DUT_IF.wg_drv_sel][31:24]; wr_data[1] == `DUT_IF.wg_silent_wave0_lim[`DUT_IF.wg_drv_sel][23:16]; wr_data[2] == `DUT_IF.wg_silent_wave0_lim[`DUT_IF.wg_drv_sel][15:8]; wr_data[3] == `DUT_IF.wg_silent_wave0_lim[`DUT_IF.wg_drv_sel][7:0];});
    					`nnc_info("SOC_TEST", "Set silent period", NNC_LOW)//0x0000_03E8 (1000us)
    					`WR_BURST_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.wr_data);
				end
				else if (j==6) begin
				...
				// --------------------------------------------------------
    				...
    				// --------------------------------------------------------
    				assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_INT_REG02 + (`DUT_IF.wg_drv_sel*`WAVEGEN_1_ADDR_BASE)); no_of_bytes == 2; wr_data[0] == 8'h00; wr_data[1] == 8'h21;});
				...
				if (j>0) // if s is 1, then we are in the negative half of the wave, no need to update the period. it has already been updated
				    j = j - 1;
				else
				    j = `NUM_PERIODS-1;
			end
			...
			// ------------------------------------------------------------------
    			// Write to ADDR_WG_DRV_INT_REG01 (clear the first address interrupt)
    			// ------------------------------------------------------------------
    			assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_INT_REG01 + (`DUT_IF.wg_drv_sel*`WAVEGEN_1_ADDR_BASE)); wr_data[0] == 8'h03;});
    			`nnc_info("SOC_TEST", "Clear the first address interrupt", NNC_LOW)
    			`WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);
		end
			...
			// --------------------------------------------------------
    			// Write burst starting from ADDR_WG_DRV_INT_REG02
    			// --------------------------------------------------------
    			assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_INT_REG02 + (`DUT_IF.wg_drv_sel*`WAVEGEN_1_ADDR_BASE)); no_of_bytes == 2; wr_data[0] == 8'h20; wr_data[1] == 8'h01;});
		...
		if (j<`NUM_PERIODS-1) //counting the bursts
		    j = j + 1;
		else if (j>=`NUM_PERIODS-1)
		    j = 0;*/
	    end
	  end
	//end
