...
//--------------------------------------------------------------------------------------
// File Name	: soc_pinmux_debugmode_atm6_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_pinmux_debugmode_atm6_test                                             
// Designer	: ddang@nanochap.com                                                                 
// Date		: 29-11-1623                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_pinmux_debugmode_atm6_test
`define TESTCFG soc_pinmux_debugmode_atm6_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_pinmux_debugmode_atm6_test_cfg");
    super.new(name);
  ...
  // -----------------------------------------------
  // testmode_sel[1:0] : 00-Normal mode, 01: Scanmode, 10: BIST mode, 11 ATM6/1/2/3/4
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_pinmux_debugmode_atm6_test start", NNC_LOW)
...
`ifndef FPGA
    ...
    // ---------------------------------------------------- 
    // Part I: Configure chip to ATM6
    // ----------------------------------------------------       
    force `SOC_TOP.IOBUF_PAD[14:10] = 3'b00110;
   ...
   // ------------------------------------------------------ 
   // Part II: Checking ATM6 will be asserted in interface
   // ------------------------------------------------------
    #100000ns;
...
`endif
    ...
    if (top_test_cfg.atm !== (30'b10_0000_0000_0000_0000_0000_0000_0000 >> `SOC_TOP.IOBUF_PAD[14:10]))
      begin
        `nnc_error("ATM6", $sformatf("ATM[0:29] = %b is not as expectation of ATM = %b", top_test_cfg.atm, (30'b10_0000_0000_0000_0000_0000_0000_0000 >> `SOC_TOP.IOBUF_PAD[14:10])))
      end  

    //Check default signal  
    if (`ANA_TOP.D2A_BIST_SEL !== 5'b00110)
      begin
        `nnc_error("ATM6", $sformatf("D2A_BIST_SEL = %b is not as expectation of 5'b00110", `ANA_TOP.D2A_BIST_SEL))
      end
    if (`ANA_TOP.D2A_DRIVER_CUR_EN !== 1'b1)
      begin
        `nnc_error("ATM6", $sformatf("D2A_DRIVER_CUR_EN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_DRIVER_CUR_EN))
      end   
    if (`ANA_TOP.D2A_STIMU_EN !== 1'b1)
      begin
        `nnc_error("ATM6", $sformatf("D2A_STIMU_EN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_STIMU_EN))
      end 
    if (`ANA_TOP.D2A_BIST_EN !== 1'b1)
      begin
        `nnc_error("ATM6", $sformatf("D2A_BIST_EN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_BIST_EN))
      end  
    if (`ANA_TOP.D2A_OSC8MHZEN !== 1'b1)
      begin
        `nnc_error("ATM6", $sformatf("D2A_OSC8MHZEN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_OSC8MHZEN))
      end 
    if(`ANA_TOP.D2A_BGBUFFER_CPTEST_EN != 1'b1)
      begin
        `nnc_error("ATM6", $sformatf("D2A_BGBUFFER_CPTEST_EN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_BGBUFFER_CPTEST_EN))
      end
...
`ifdef BEHAVIORAL             
        if (`ANA_WRAPPER_TOP.pinmux_if.D2A_TRIM_SIG[6][7:0]!== {rand_num[7:0]}) begin
        `nnc_error("ATM6", $sformatf("`SOC_TOP.pinmux_if.D2A_TRIM_SIG[6][7:0] = %b is not as expectation of = %b", `ANA_WRAPPER_TOP.pinmux_if.D2A_TRIM_SIG[6][7:0], {rand_num[7:0]}));
        end
`else
        if (`ANA_WRAPPER_TOP.pinmux_if_D2A_TRIM_SIG[7:0]!== {rand_num[7:0]}) begin
        `nnc_error("ATM6", $sformatf("`SOC_TOP.pinmux_if.D2A_TRIM_SIG[7:0] = %b is not as expectation of = %b", `ANA_WRAPPER_TOP.pinmux_if_D2A_TRIM_SIG[7:0], {rand_num[7:0]}));
        end
`endif
        if (`ANA_TOP.D2A_DRIVER_CUR_TRIM!== {rand_num[7:0]}) begin
        `nnc_error("ATM6", $sformatf("`ANA_TOP.D2A_DRIVER_CUR_TRIM = %b is not as expectation of = %b", `ANA_TOP.D2A_DRIVER_CUR_TRIM, {rand_num[7:0]}));
        end
    ...
    for (int i=0; i < 100; i++) begin
        ...
        `nnc_error("ATM6", $sformatf("`EPROM_TOP.unlock_gpio:%b is not as expectation of CLKSEL: %b",`EPROM_TOP.unlock_gpio, rand_bit))
        end
    ...
    for (int i=0; i < 100; i++) begin
        ...
        `ifndef POSTLAYOUT_PG
           ...
           `nnc_error("ATM6", $sformatf("`RST_CTRL_TOP.ext_resetn:%b is not as expectation of RESETn: %b",`RST_CTRL_TOP.ext_resetn, rand_bit))
           end
           if (`RST_CTRL_TOP.scan_rst_n !== rand_bit) begin
           `nnc_error("ATM6", $sformatf("`RST_CTRL_TOP.scan_rst_n:%b is not as expectation of RESETn: %b",`RST_CTRL_TOP.scan_rst_n, rand_bit))
           end
           if (`RST_CTRL_TOP.otp_bist_resetn !== rand_bit) begin
           `nnc_error("ATM6", $sformatf("`RST_CTRL_TOP.otp_bist_resetn:%b is not as expectation of RESETn: %b",`RST_CTRL_TOP.otp_bist_resetn, rand_bit))
           end
        `else //in postlayout above all reset signals of pinmux has been optimized so consider only below one
           ...
           `nnc_error("ATM6", $sformatf("`DIG_TOP.u_pinmux.iopad_resetn_y:%b is not as expectation of RESETn: %b",`DIG_TOP.u_pinmux.iopad_resetn_y, rand_bit))
           end
           if (`RST_CTRL_TOP.otp_bist_resetn!== rand_bit) begin
             `nnc_error("ATM6", $sformatf("`DIG_TOP.u_pinmux.otp_bist_resetn = %b is not as expectation of = %b", `RST_CTRL_TOP.otp_bist_resetn, rand_bit));
           end
           if (`DIG_TOP.scan_rst_n !== rand_bit) begin
            `nnc_error("ATM6", $sformatf("`DIG_TOP.u_pinmux.scan_rst_n = %b is not as expectation of scan_rst_n = %b",`DIG_TOP.scan_rst_n, `SOC_TB.scan_rst_n))
          end
        `endif
...
`ifdef BEHAVIORAL                         
    for (int i=0; i < 100; i++) begin
        ...
        `nnc_error("ATM6", $sformatf("`CLK_CTRL_TOP.ext_hfclk : %b is not as expectation of EXT_CLK: %b",`CLK_CTRL_TOP.ext_hfclk, rand_bit))
        end
...
`endif
    ...
    for (int i=0; i < 100; i++) begin
        ...
        `nnc_error("ATM6", $sformatf("`DIG_TOP.IOBUF_PAD[9] : %b is not as expectation of `DIG_TOP.u_pinmux.i_otp_vpp_en: %b",`SOC_TOP.IOBUF_PAD[9], rand_bit))
        end
  ...
  /*    
    ...
    // ---------------------------------------------------------
    //Check default signal           
    if (`ANA_TOP.D2A_BIST_SEL !== 4'b0110)
      begin
        `nnc_error("ATM6", $sformatf("D2A_BIST_SEL = %b is not as expectation of 4'b0110", `ANA_TOP.D2A_BIST_SEL))
      end  

    if (`ANA_TOP.D2A_IDAC_EN_CH1 !== 1'b1)
      begin
        `nnc_error("ATM6", $sformatf("D2A_IDAC_EN_CH1 = %b is not as expectation of 1'b1", `ANA_TOP.D2A_IDAC_EN_CH1))
      end
    ...
    if (`ANA_TOP.D2A_OSC2MHZEN !== 1'b0)
      begin
        `nnc_error("ATM6", $sformatf("D2A_OSC2MHZEN = %b is not as expectation of 1'b0", `ANA_TOP.D2A_OSC2MHZEN))
      end  

    if (`ANA_TOP.D2A_TSC_EN_CH1 !== 1'b1)
      begin
        `nnc_error("ATM6", $sformatf("D2A_TSC_EN_CH1 = %b is not as expectation of 1'b1", `ANA_TOP.D2A_TSC_EN_CH1))
      end 

    if (`ANA_TOP.D2A_TSC_COMP_EN_CH1 !== 1'b1)
      begin
        `nnc_error("ATM6", $sformatf("D2A_TSC_COMP_EN_CH1 = %b is not as expectation of 1'b1", `ANA_TOP.D2A_TSC_COMP_EN_CH1))
      end  

    if (`ANA_TOP.D2A_VDAC8B_EN_CH1 !== 1'b1)
      begin
        `nnc_error("ATM6", $sformatf("D2A_VDAC8B_EN_CH1 = %b is not as expectation of 1'b1", `ANA_TOP.D2A_VDAC8B_EN_CH1))
      end  
   
    // ------------------------------------------------------------ 
    // Part IV: Checking the connections from PADs to ANA Interface
    // ------------------------------------------------------------   
    // Checking D2A_IDAC_TSC_COMP_OUT_SEL pin
    for (int i=0; i < 100; i++) begin
        force `SOC_TOP.CLKSEL = $random;
        #10000ns;
        rand_bit = `SOC_TOP.CLKSEL;
...
`ifdef BEHAVIORAL                             
    ...
    for (int i=0; i < 100; i++) begin
        ...
        `nnc_error("ATM6", $sformatf("`DIG_TOP.u_pinmux.ext_clk : %b is not as expectation of EXT_CLK: %b",`DIG_TOP.u_pinmux.ext_clk, rand_bit))
        end
      release `SOC_TOP.IOBUF_PAD[0];                      
    end  
`endif
    ...
    for (int i=0; i < 100; i++) begin
        ...
        `ifndef POSTLAYOUT_PG
           ...
           `nnc_error("ATM6", $sformatf("`DIG_TOP.u_pinmux.pin_rstn:%b is not as expectation of RESETn: %b",`DIG_TOP.u_pinmux.pin_rstn, rand_bit))
           end
           if (`DIG_TOP.u_pinmux.scan_rst_n !== rand_bit) begin
           `nnc_error("ATM6", $sformatf("`DIG_TOP.u_pinmux.scan_rst_n:%b is not as expectation of RESETn: %b",`DIG_TOP.u_pinmux.scan_rst_n, rand_bit))
           end
           if (`DIG_TOP.u_pinmux.otp_bist_resetn !== rand_bit) begin
           `nnc_error("ATM6", $sformatf("`DIG_TOP.u_pinmux.otp_bist_resetn:%b is not as expectation of RESETn: %b",`DIG_TOP.u_pinmux.otp_bist_resetn, rand_bit))
           end
        `else //in postlayout above all reset signals of pinmux has been optimized so consider only below one
           if (`DIG_TOP.u_pinmux.iopad_resetn_y !== rand_bit) begin
...
`ifdef BEHAVIORAL             
        if (`ANA_WRAPPER_TOP.pinmux_if.D2A_TRIM_SIG[6][7:0]!== {1'b0,rand_num[6:0]}) begin
        `nnc_error("ATM6", $sformatf("`SOC_TOP.pinmux_if.D2A_TRIM_SIG[6][7:0] = %b is not as expectation of = %b", `ANA_WRAPPER_TOP.pinmux_if.D2A_TRIM_SIG[6][7:0], {1'b0,rand_num[6:0]}));
        end                            
`else
        if (`ANA_WRAPPER_TOP.pinmux_if_D2A_TRIM_SIG[55:48]!== {1'b0,rand_num[6:0]}) begin
        `nnc_error("ATM6", $sformatf("`SOC_TOP.pinmux_if.D2A_TRIM_SIG[54:48] = %b is not as expectation of = %b", `ANA_WRAPPER_TOP.pinmux_if_D2A_TRIM_SIG[55:48], {1'b0,rand_num[6:0]}));
        end
`endif

        if (`ANA_TOP.D2A_IBIAS_IDAC_TRIM !== {1'b0, rand_num[2:0]}) begin
        `nnc_error("ATM6", $sformatf("`ANA_TOP.D2A_IBIAS_IDAC_TRIM = %b is not as expectation of = %b", `ANA_TOP.D2A_IBIAS_IDAC_TRIM, {1'b0, rand_num[2:0]}));
        end

        if (`ANA_TOP.D2A_TSC_TRIM_CH1 !== rand_num[6:3]) begin
        `nnc_error("ATM6", $sformatf("`ANA_TOP.D2A_TSC_TRIM_CH1 = %b is not as expectation of = %b", `ANA_TOP.D2A_TSC_TRIM_CH1, rand_num[6:3]));
        end
