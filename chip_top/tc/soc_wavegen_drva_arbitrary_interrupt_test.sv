...
//--------------------------------------------------------------------------------------
// File Name	: soc_wavegen_drva_arbitrary_interrupt_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_wavegen_drva_arbitrary_interrupt_test                                             
...
//        This test also constraint the bit[6] of control register to be 1, so that the 128 points
//        can be loaded with arbitrary data. But this test enables interrupt and load data upon 
//        interrupt to generate arbitrary waveform.
//*************************************************************************************************

// =================================================
// Testcase name is defined
// -------------------------------------------------
`define TESTNAME soc_wavegen_drva_arbitrary_interrupt_test
`define TESTCFG soc_wavegen_drva_arbitrary_interrupt_test_cfg
...
class `TESTCFG extends soc_wavegen_base_test_cfg;
  ...
  function new (string name = "soc_wavegen_drva_arbitrary_interrupt_test_cfg");
    super.new(name);
...
class `TESTNAME extends soc_wavegen_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_wavegen_drva_arbitrary_interrupt_test start", NNC_LOW)
    ...
    // --------------------------------------------------------
    #1ms;
    `nnc_info("SOC_TEST", "soc_wavegen_drva_arbitrary_interrupt_test end now", NNC_LOW)
...
`endif
	if(`DUT_IF.intr_length_slct_level_or_pulse === 0) begin
	  if(`DUT_IF.int_active_level_high_or_low === 1)//active high
		...
		@(posedge top_test_cfg.INTB_pulse_flag);
