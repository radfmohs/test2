...
//--------------------------------------------------------------------------------------
// File Name	: soc_wavegen_drva_scale_offset_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_wavegen_drva_scale_offset_test                                             
...
//*******************************************************************************
// NOTE : This test follows the same sequence as basic sine/pulse/triangle test, 
//        but the positive/negative scaling and offset are applied additionally  
//*******************************************************************************

// =================================================
// Testcase name is defined
// -------------------------------------------------
`define TESTNAME soc_wavegen_drva_scale_offset_test
`define TESTCFG soc_wavegen_drva_scale_offset_test_cfg
...
class `TESTCFG extends soc_wavegen_base_test_cfg;
  ...
  rand logic       dac_bit_len_sel;//1'b0:8-bits; 1'b1:12-bits (offset and scaling is not applicable for 12 bits selection)
  ...
  rand logic       neg_scale_up_down;
  rand logic       pos_scale_up_down;
  rand logic [6:0] neg_scale_val;
  rand logic [6:0] pos_scale_val;
  rand logic       dac0_multi_argo_ctrl;//1'b0: (dac_8bit*16>>dac_msb_sel)+OFFSET; 1'b1: (dac_8bit*SCALE)+OFFSET; (only for scaling up/multiplication control)
  rand logic       dac1_multi_argo_ctrl;//1'b0: (dac_8bit*16>>dac_msb_sel)+OFFSET; 1'b1: (dac_8bit*SCALE)+OFFSET; (only for scaling up/multiplication control)
       logic [7:0] pulse_data;
  // -----------------------------------------------
  // End of decalration of new variables 
  // ===============================================

  function new (string name = "soc_wavegen_drva_scale_offset_test_cfg");
    super.new(name);
  ...
  //preload_sel
  constraint c_preload_sel     { preload_sel inside {[0:3]}; ((waveform_sel == 3'b001) || (waveform_sel == 3'b010)) -> preload_sel == 2'b11;}
  ...
  //points_sel
  constraint c_points_sel      { ((waveform_sel == 3'b000) && (pos_neg_diff_sel == 1'b0)) -> points_sel inside {[0:6]};
                                 ((waveform_sel == 3'b000) && (pos_neg_diff_sel == 1'b1)) -> points_sel inside {[0:7]};
                                 ((waveform_sel == 3'b001) && (pos_neg_diff_sel == 1'b0)) -> points_sel inside {[1:6]};
                                 ((waveform_sel == 3'b001) && (pos_neg_diff_sel == 1'b1)) -> points_sel inside {[0:6]};
                                 ((waveform_sel == 3'b010) && (pos_neg_diff_sel == 1'b0)) -> points_sel inside {[2:6]};
                                 ((waveform_sel == 3'b010) && (pos_neg_diff_sel == 1'b1)) -> points_sel inside {[1:6]};
                               }
  ...
  //neg_scale_val
  constraint c_neg_scale_val   { (neg_scale_up_down == 0) -> neg_scale_val inside {[1:16]}; (neg_scale_up_down == 1) -> neg_scale_val inside {[1:7]};}

  //pos_scale_val
  constraint c_pos_scale_val   { (pos_scale_up_down == 0) -> pos_scale_val inside {[1:16]}; (pos_scale_up_down == 1) -> pos_scale_val inside {[1:7]};}
  ...
  //dac0_multi_argo_ctrl
  //constraint c_dac0_multi_argo_ctrl { dac0_multi_argo_ctrl == 0;}

  //dac1_multi_argo_ctrl
  //constraint c_dac1_multi_argo_ctrl { dac1_multi_argo_ctrl == 0;}
...
class `TESTNAME extends soc_wavegen_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_wavegen_drva_scale_offset_test start", NNC_LOW)
    ...
    //wavegen_calc_clock_num(clk_freq (KHz), rest_t (us), silent_t (us), hlf_wave_per (us), neg_hlf_wave_per (us))
    wavegen_calc_clock_num(top_test_cfg.clk_freq, 100, 1000, top_test_cfg.half_period1, top_test_cfg.half_period1);
    ...
    //wavegen_calc_clock_num(clk_freq (KHz), rest_t (us), silent_t (us), hlf_wave_per (us), neg_hlf_wave_per (us))
    wavegen_calc_clock_num(top_test_cfg.clk_freq, 100, 1000, top_test_cfg.half_period2, top_test_cfg.half_period2);
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == `SOC_ADDR_WG_DRV_CTRL0_REG; (PRELOAD == 2'b01) -> dac_bit_len_sel == 1'b0; wr_data[0] == {2'b0, dac_bit_len_sel,auto_man, 4'b0};});
    `WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);

    // --------------------------------------------------------
    // Write to SOC_ADDR_WG_DRV_CTRL0_REG for drv1
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_CTRL0_REG + `WAVEGEN_1_ADDR_BASE); (PRELOAD == 2'b01) -> dac_bit_len_sel == 1'b0; wr_data[0] == {2'b0, dac_bit_len_sel,auto_man, 4'b0};});
    `WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);

    // --------------------------------------------------------
    // Write burst starting from SOC_ADDR_WG_DRV_CTRL1_REG for drv0
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == `SOC_ADDR_WG_DRV_CTRL1_REG; no_of_bytes == 2; wr_data[0] == {dac0_multi_argo_ctrl, dac0_msb_sel, dac0_data_h}; wr_data[1] == dac0_data_l;});
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_CTRL1_REG + `WAVEGEN_1_ADDR_BASE); no_of_bytes == 2; wr_data[0] == {dac1_multi_argo_ctrl, dac1_msb_sel, dac1_data_h}; wr_data[1] == dac1_data_l;});
    `WR_BURST_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.wr_data);

    wavegen_drv_config(`WAVEGEN_0_ADDR_BASE);
    wavegen_drv_config(`WAVEGEN_1_ADDR_BASE);

    `DUT_IF.scale_en = 1'b1;
    `nnc_info("SOC_TEST", $sformatf("enabling chip_0 wavegen sb now"), NNC_LOW)
    // Disable the specifc scale offset check inside wavgegen monitor for Netlist, as it checks for the internal signals from design which are not present in netlist     
    `ifndef BEHAVIORAL
    `WAVEGEN_SCB_SCALE_OFFSET_CHECK_EN = 0;
    `endif
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == `SOC_ADDR_WG_DRV_CTRL_REG0; wr_data[0] == {POS_NEG_DIFF,LOAD_POINTS,NO_OF_WAVEFORMS,PRELOAD,1'b1};});
    ...
    else if(top_test_cfg.PRELOAD === 2'b10)
    	`nnc_info("SOC_TEST", "Enable driver0 using control register with preloaded triangle values", NNC_LOW)
    ...
    `WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_CTRL_REG0 + `WAVEGEN_1_ADDR_BASE); wr_data[0] == {POS_NEG_DIFF,LOAD_POINTS,NO_OF_WAVEFORMS,PRELOAD,1'b1};});
    if(top_test_cfg.PRELOAD === 2'b00)
    	...
    	`nnc_info("SOC_TEST", "Enable driver1 using control register with preloaded triangle values", NNC_LOW)
    else if(top_test_cfg.PRELOAD === 2'b11)
    ...
    // --------------------------------------------------------
    #50ms;
    `nnc_info("SOC_TEST", "soc_wavegen_drva_scale_offset_test end now", NNC_LOW)
  ...
  begin
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_CONFIG_REG0 + WG_BASE); wr_data[0] == {pos_dis, 1'b1, 1'b0, 1'b0, 1'b1, 1'b0, neg_ena, 1'b0};});
    `nnc_info("SOC_TEST", "Set driver configuration register", NNC_LOW)
    `WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);
    
    // --------------------------------------------------------
    // Write to ADDR_WG_DRV_NEG_OFFSET_REG0
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_NEG_OFFSET_REG0 + WG_BASE); wr_data[0] == 8'h06;});
    `nnc_info("SOC_TEST", "Set NEG Offset value", NNC_LOW)
    `WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);

    // --------------------------------------------------------
    // Write to ADDR_WG_DRV_NEG_SCALE_REG0 (By default it is 1)
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_NEG_SCALE_REG0 + WG_BASE); wr_data[0] == {neg_scale_up_down, neg_scale_val};});
    `nnc_info("SOC_TEST", "Set NEG Scale value", NNC_LOW)
    `WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);
    
    // --------------------------------------------------------
    // Write to ADDR_WG_DRV_POS_OFFSET_REG0
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_POS_OFFSET_REG0 + WG_BASE); wr_data[0] == 8'h12;});
    `nnc_info("SOC_TEST", "Set POS Offset value", NNC_LOW)
    `WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);

    // --------------------------------------------------------
    // Write to ADDR_WG_DRV_POS_SCALE_REG0
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_POS_SCALE_REG0 + WG_BASE); wr_data[0] == {pos_scale_up_down, pos_scale_val};});
    `nnc_info("SOC_TEST", "Set POS Scale value", NNC_LOW)
    `WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_POINT_CONFIG + WG_BASE); wr_data[0] == NO_OF_POINTS;});
    `WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);
	...
	// --------------------------------------------------------
    	...
    	// --------------------------------------------------------
	top_test_cfg.pulse_data = 8'h4F;
	assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_IN_WAVE_REG01 + WG_BASE); wr_data[0] == pulse_data;});
    	`WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);
    end
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_PULLBA_REG + WG_BASE); wr_data[0] == PULLAB_CTRL;});
