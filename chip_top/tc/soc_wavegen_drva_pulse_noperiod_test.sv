...
//--------------------------------------------------------------------------------------
// File Name	: soc_wavegen_drva_pulse_noperiod_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_wavegen_drva_pulse_noperiod_test                                             
...
//***************************************************************************************
// NOTE : This test follows the same configuration sequence as pulse test, but the 
//        intention of this test is to observe the effect of 0 period setting in 
//        positive/negative time, rest time and silent time.
//***************************************************************************************

// =================================================
// Testcase name is defined
// -------------------------------------------------
`define TESTNAME soc_wavegen_drva_pulse_noperiod_test
`define TESTCFG soc_wavegen_drva_pulse_noperiod_test_cfg
...
class `TESTCFG extends soc_wavegen_base_test_cfg;
  ...
  rand logic [1:0] test_sel;//00: set pos_period 0; 01: set neg_period 0; 10: set rest_period 0; 11: set silent_period 0
  // -----------------------------------------------
  // End of decalration of new variables 
  // ===============================================

  function new (string name = "soc_wavegen_drva_pulse_noperiod_test_cfg");
    super.new(name);
  ...
  //pos_neg_diff_sel
  constraint c_pos_neg_diff_sel { ((test_sel == 2'b00) || (test_sel == 2'b01)) -> pos_neg_diff_sel == 1'b0;}
...
class `TESTNAME extends soc_wavegen_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_wavegen_drva_pulse_noperiod_test start", NNC_LOW)
    ...
    `DUT_IF.noperiod_pos_neg_sel = top_test_cfg.test_sel;
    ...
    if(`DUT_IF.noperiod_pos_neg_sel === 2'b00)
	`nnc_info("SOC_TEST", "POS PERIOD:0", NNC_LOW)
    else if(`DUT_IF.noperiod_pos_neg_sel === 2'b01)
	`nnc_info("SOC_TEST", "NEG PERIOD:0", NNC_LOW)
    else if(`DUT_IF.noperiod_pos_neg_sel === 2'b10)
	`nnc_info("SOC_TEST", "REST PERIOD:0", NNC_LOW)
    else
	`nnc_info("SOC_TEST", "SILENT PERIOD:0", NNC_LOW)
    ...
    `nnc_info("SOC_TEST", $sformatf("******** Driver (%d) WAVE 0 ******** CLK_FREQ: %dKhz, HALF_PERIOD_LIMIT: %dus, HALF_PERIOD_TARGET: %dus, POS_HALF_PERIOD_CLKS_PER_POINT: %d, NEG_HALF_PERIOD_CLKS_PER_POINT: %d, REST_PERIOD_CLKS: %d, SILENT_PERIOD_CLKS: %d", i, top_test_cfg.clk_freq, top_test_cfg.half_period_limit, top_test_cfg.half_period0, `DUT_IF.wg_hlf_wave0_lim[i], `DUT_IF.wg_neg_hlf_wave0_lim[i], `DUT_IF.wg_rest_wave0_lim[i], `DUT_IF.wg_silent_wave0_lim[i]), NNC_LOW)
    ...
    `nnc_info("SOC_TEST", $sformatf("******** Driver (%d) WAVE 1 ******** CLK_FREQ: %dKhz, HALF_PERIOD_LIMIT: %dus, HALF_PERIOD_TARGET: %dus, POS_HALF_PERIOD_CLKS_PER_POINT: %d, NEG_HALF_PERIOD_CLKS_PER_POINT: %d, REST_PERIOD_CLKS: %d, SILENT_PERIOD_CLKS: %d", i, top_test_cfg.clk_freq, top_test_cfg.half_period_limit, top_test_cfg.half_period1, `DUT_IF.wg_hlf_wave1_lim[i], `DUT_IF.wg_neg_hlf_wave1_lim[i], `DUT_IF.wg_rest_wave1_lim[i], `DUT_IF.wg_silent_wave1_lim[i]), NNC_LOW)
    ...
    `nnc_info("SOC_TEST", $sformatf("******** Driver (%d) WAVE 2 ******** CLK_FREQ: %dKhz, HALF_PERIOD_LIMIT: %dus, HALF_PERIOD_TARGET: %dus, POS_HALF_PERIOD_CLKS_PER_POINT: %d, NEG_HALF_PERIOD_CLKS_PER_POINT: %d, REST_PERIOD_CLKS: %d, SILENT_PERIOD_CLKS: %d", i, top_test_cfg.clk_freq, top_test_cfg.half_period_limit, top_test_cfg.half_period2, `DUT_IF.wg_hlf_wave2_lim[i], `DUT_IF.wg_neg_hlf_wave2_lim[i], `DUT_IF.wg_rest_wave2_lim[i], `DUT_IF.wg_silent_wave2_lim[i]), NNC_LOW)
    end
    ...
    // --------------------------------------------------------
    #50ms;
    `nnc_info("SOC_TEST", "soc_wavegen_drva_pulse_noperiod_test end now", NNC_LOW)
  ...
  begin
    if(`DUT_IF.noperiod_pos_neg_sel === 2'b00)
	top_test_cfg.hlf_wave_lim = 32'b0;
    else
    	top_test_cfg.hlf_wave_lim = (hlf_wave_per * {20'b0,clk_freq}) / 1000;
    if(`DUT_IF.noperiod_pos_neg_sel === 2'b01)
    	top_test_cfg.neg_hlf_wave_lim = 32'b0;
    else
    	top_test_cfg.neg_hlf_wave_lim = (neg_hlf_wave_per * {20'b0,clk_freq}) / 1000;
    if(`DUT_IF.noperiod_pos_neg_sel === 2'b10)
	top_test_cfg.rest_lim = 16'b0;
    else
    	top_test_cfg.rest_lim = ({8'b0,rest_t} * {4'b0,clk_freq}) / 1000;
    if(`DUT_IF.noperiod_pos_neg_sel === 2'b11)
	top_test_cfg.silent_lim = 32'b0;
    else
  ...
  begin
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_CONFIG_REG0 + WG_BASE); wr_data[0] == {top_test_cfg.pos_dis, 1'b1, 1'b0, 1'b0, 1'b1, 1'b1, top_test_cfg.neg_ena, 1'b1};});
