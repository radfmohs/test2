...
//--------------------------------------------------------------------------------------
// File Name	: soc_wavegen_pair_drv_arb_preload_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_wavegen_pair_drv_arb_preload_test                                             
...
// -------------------------------------------------
`define TESTNAME soc_wavegen_pair_drv_arb_preload_test
`define TESTCFG soc_wavegen_pair_drv_arb_preload_test_cfg
...
class `TESTCFG extends soc_wavegen_pair_drv_base_test_cfg;
  ...
  function new (string name = "soc_wavegen_pair_drv_arb_preload_test_cfg");
    super.new(name);
  ...
  //python_check_en
  constraint c_python_check_en { wg_sine_en == 1 -> python_check_en == ($countones(wavegen_drv_en[15:0]) == 2) ;
                                 wg_sine_en == 0 -> python_check_en == 0;
                               }

  //preload_sel
  constraint c_preload_sel     { waveform_sel == 3'b001 -> preload_sel inside {0,1,2}; 
                                 waveform_sel == 3'b010 -> preload_sel inside {0,1};
  ...
  */
  ...
  constraint c_waveform_sel    { waveform_sel inside {[1:2]};
                               }
...
class `TESTNAME extends soc_wavegen_pair_drv_base_test;
  ...
  virtual task pre_reset_phase(nnc_phase phase);
    ...
    `DUT_IF.wg_preload_sel = top_test_cfg.preload_sel;
    `DUT_IF.wg_load_points_sel = top_test_cfg.load_points_sel;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_wavegen_pair_drv_arb_preload_test start", NNC_LOW)
    ...
    // --------------------------------------------------------
    #10000ns;
    `nnc_info("SOC_TEST", "soc_wavegen_pair_drv_arb_preload_test end now", NNC_LOW)
