...
//--------------------------------------------------------------------------------------
// File Name	: soc_wavegen_drva_sawtooth_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_wavegen_drva_sawtooth_test                                             
...
//***************************************************************************************
// NOTE : The test is intented  to generate triangle wave for driver1 & driver2
//***************************************************************************************

// =================================================
// Testcase name is defined
// -------------------------------------------------
`define TESTNAME soc_wavegen_drva_sawtooth_test
`define TESTCFG soc_wavegen_drva_sawtooth_test_cfg
...
class `TESTCFG extends soc_wavegen_base_test_cfg;
  ...
  rand logic       dac_bit_len_sel;//1'b0:8-bits; 1'b1:12-bits (only 8 bits supported for sawtooth)
  ...
  function new (string name = "soc_wavegen_drva_sawtooth_test_cfg");
    super.new(name);
  ...
  //python_check_en
  constraint c_python_check_en { python_check_en inside {[0:1]}; }

  //preload_sel
  constraint c_preload_sel     { preload_sel inside {2,3};}
  ...
  //load_points_sel
  constraint c_load_points_sel { (((waveform_sel == 3'b001) || (waveform_sel == 3'b010)) && (preload_sel == 2'b11)) -> load_points_sel == 1'b1; (preload_sel == 2'b10) -> load_points_sel == 1'b0;}
...
class `TESTNAME extends soc_wavegen_base_test;
  ...
  virtual task pre_reset_phase(nnc_phase phase);
    ...
    `DUT_IF.waveshape_sel = 2'b10;//indicates triangle wave
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_wavegen_drva_sawtooth_test start", NNC_LOW)
    ...
    `nnc_info("SOC_TEST", "soc_wavegen_drva_sawtooth_test end now", NNC_LOW)
  ...
  begin
    ...
    // Interface
    if(chip_num === 0) begin
    	`DUT_IF.wavegen_sample_num_per_period = top_test_cfg.NO_OF_LOAD_POINTS * 2;//no: of samples in 1 sawtooth wave
    	`DUT_IF.python_length = `DUT_IF.wavegen_sample_num_per_period * top_test_cfg.NO_OF_WAVES;//send N sawtooth waves to python
    end
    else begin
    	`DUT_IF.wavegen_sample_num_per_period_chip1 = top_test_cfg.NO_OF_LOAD_POINTS * 2;//no: of samples in 1 sawtooth wave
    	`DUT_IF.python_length_chip1 = `DUT_IF.wavegen_sample_num_per_period_chip1 * top_test_cfg.NO_OF_WAVES;//send N sawtooth waves to python
    end
  ...
  begin
    ...
    if(top_test_cfg.PRELOAD === 2'b10)
    	`nnc_info("SOC_TEST", "Config driver control register with preloaded triangular values", NNC_LOW)
