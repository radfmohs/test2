...
//--------------------------------------------------------------------------------------
// File Name	: soc_wavegen_drva_pulse_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_wavegen_drva_pulse_test                                             
...
//***************************************************************************************
// NOTE : The test is intented  to generate pulse wave for driver1 & driver2
//***************************************************************************************

// =================================================
// Testcase name is defined
// -------------------------------------------------
`define TESTNAME soc_wavegen_drva_pulse_test
`define TESTCFG soc_wavegen_drva_pulse_test_cfg
...
class `TESTCFG extends soc_wavegen_base_test_cfg;
  ...
  function new (string name = "soc_wavegen_drva_pulse_test_cfg");
    super.new(name);
...
class `TESTNAME extends soc_wavegen_base_test;
  ...
  virtual task pre_reset_phase(nnc_phase phase);
    ...
    `DUT_IF.waveshape_sel = 2'b01;//indicates pulse wave
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_wavegen_drva_pulse_test start", NNC_LOW)
    ...
    `nnc_info("SOC_TEST", "soc_wavegen_drva_pulse_test end now", NNC_LOW)
  ...
  begin
    ...
    // Interface
    if(chip_num === 0) begin
    	`DUT_IF.wavegen_sample_num_per_period = top_test_cfg.NO_OF_LOAD_POINTS * 2;//no: of samples in 1 pulse wave
    	`DUT_IF.python_length = `DUT_IF.wavegen_sample_num_per_period * top_test_cfg.NO_OF_WAVES;//send N pulse waves to python
    end
    else begin
    	`DUT_IF.wavegen_sample_num_per_period_chip1 = top_test_cfg.NO_OF_LOAD_POINTS * 2;//no: of samples in 1 pulse wave
    	`DUT_IF.python_length_chip1 = `DUT_IF.wavegen_sample_num_per_period_chip1 * top_test_cfg.NO_OF_WAVES;//send N pulse waves to python
    end
    ...
    wavegen_calc_clock_num(top_test_cfg.clk_freq, top_test_cfg.rest_period[0], top_test_cfg.silent_period[0], top_test_cfg.half_period0[0], top_test_cfg.half_period0[1]);
    ...
    `nnc_info("SOC_TEST", $sformatf("******** Driver (%d) WAVE 0 ******** CLK_FREQ: %dKhz, HALF_PERIOD_LIMIT: %dus, POS_HALF_PERIOD_TARGET: %dus, NEG_HALF_PERIOD_TARGET: %dus, POS_HALF_PERIOD_CLKS_PER_POINT: %d, NEG_HALF_PERIOD_CLKS_PER_POINT: %d, REST_PERIOD: %dus, SILENT_PERIOD: %dus, REST_PERIOD_CLKS: %d, SILENT_PERIOD_CLKS: %d", i, top_test_cfg.clk_freq, top_test_cfg.half_period_limit, top_test_cfg.half_period0[0], top_test_cfg.half_period0[1], `DUT_IF.wg_hlf_wave0_lim[i], `DUT_IF.wg_neg_hlf_wave0_lim[i], top_test_cfg.rest_period[0], top_test_cfg.silent_period[0], `DUT_IF.wg_rest_wave0_lim[i], `DUT_IF.wg_silent_wave0_lim[i]), NNC_LOW)

    //wavegen_calc_clock_num(clk_freq (KHz), rest_t (us), silent_t (us), hlf_wave_per (us), neg_hlf_wave_per (us))
    wavegen_calc_clock_num(top_test_cfg.clk_freq, top_test_cfg.rest_period[1], top_test_cfg.silent_period[1], top_test_cfg.half_period1[0], top_test_cfg.half_period1[1]);
