...
//--------------------------------------------------------------------------------------
// File Name	: soc_wavegen_drva_sine_alternate_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_wavegen_drva_sine_alternate_test                                             
...
//***************************************************************************************
// NOTE : The test is intented to generate sine wave with alternate mode ON.
//        For alternate mode test, there is no negative side. Alternate mode 
//        has additional settings that determine the period of alternating.
//***************************************************************************************

// =================================================
// Testcase name is defined
// -------------------------------------------------
`define TESTNAME soc_wavegen_drva_sine_alternate_test
`define TESTCFG soc_wavegen_drva_sine_alternate_test_cfg
...
class `TESTCFG extends soc_wavegen_base_test_cfg;
  ...
  rand logic [15:0] alt_silent_lim;
       ...
       logic [15:0] max_alt_lim;
  // -----------------------------------------------
  // End of decalration of new variables 
  // ===============================================

  function new (string name = "soc_wavegen_drva_sine_alternate_test_cfg");
    super.new(name);
  ...
  //points_sel
  constraint c_points_sel      { (load_points_sel == 1'b0) -> points_sel != 6; points_sel inside {0,7};}

  //waveform_sel
  constraint c_waveform_sel    { waveform_sel inside {[0:0]};}

  //load_points_sel
  constraint c_load_points_sel { (preload_sel == 2'b00) -> load_points_sel == 1'b0;}
  ...
  //alt_lim
  //constraint c_alt_lim         { alt_lim inside {[2:127]};}

  //alt_silent_lim
  constraint c_alt_silent_lim  { alt_silent_lim inside {[0:127]};}
...
class `TESTNAME extends soc_wavegen_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_wavegen_drva_sine_alternate_test start", NNC_LOW)
    ...
    // --------------------------------------------------------
    #100ms;
    `nnc_info("SOC_TEST", "soc_wavegen_drva_sine_alternate_test end now", NNC_LOW)
  ...
  begin
    ...
    // --------------------------------------------------------
    top_test_cfg.max_alt_lim = (top_test_cfg.NO_OF_POINTS * `DUT_IF.wg_hlf_wave0_lim[`DUT_IF.wg_drv_sel])/4;//considering atleast 4 alternating waves in 1 half cycle
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_ALT_LIM_REG01 + WG_BASE); no_of_bytes == 2; alt_lim inside {[16'h2:max_alt_lim]}; wr_data[0] == alt_lim[15:8]; wr_data[1] == alt_lim[7:0];});
