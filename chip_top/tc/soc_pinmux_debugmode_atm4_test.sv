...
//--------------------------------------------------------------------------------------
// File Name	: soc_pinmux_debugmode_atm4_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_pinmux_debugmode_atm4_test                                             
// Designer	: ddang@nanochap.com                                                                 
// Date		: 29-11-1623                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_pinmux_debugmode_atm4_test
`define TESTCFG soc_pinmux_debugmode_atm4_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_pinmux_debugmode_atm4_test_cfg");
    super.new(name);
  ...
  // -----------------------------------------------
  // testmode_sel[1:0] : 00-Normal mode, 01: Scanmode, 10: BIST mode, 11 ATM4/1/2/3/4
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_pinmux_debugmode_atm4_test start", NNC_LOW)
...
/*    force `EPROM_IP.power_ready = 1'b0;
   ...
   // ------------------------------------------------------ 
   // Part II: Checking ATM4 will be asserted in interface
   // ------------------------------------------------------
     #100000ns;
  ...
  `ifdef BEHAVIORAL
    ...
    if (top_test_cfg.atm !== (30'b10_0000_0000_0000_0000_0000_0000_0000 >> `SOC_TOP.IOBUF_PAD[14:10]))
      begin
        `nnc_error("ATM4", $sformatf("ATM[0:29] = %b is not as expectation of ATM = %b", top_test_cfg.atm, (30'b10_0000_0000_0000_0000_0000_0000_0000 >> `SOC_TOP.IOBUF_PAD[14:10])))
      end  
  `endif
...
`else
        if (`ANA_WRAPPER_TOP.pinmux_if_D2A_TRIM_SIG[39:32]!== {rand_num[7:0]}) begin
        `nnc_error("ATM4", $sformatf("`SOC_TOP.pinmux_if_D2A_TRIM_SIG[39:32] = %b is not as expectation of = %b", `ANA_WRAPPER_TOP.pinmux_if_D2A_TRIM_SIG[39:32], {rand_num[7:0]}));
        end
`endif
...
/*  
    ...
    if (`ANA_TOP.D2A_VDAC_EN_CH1 !== 1'b1)
      begin
        `nnc_error("ATM4", $sformatf("D2A_VDAC_EN_CH1 = %b is not as expectation of 1'b1", `ANA_TOP.D2A_VDAC_EN_CH1))
      end  
    ...
    if (`ANA_TOP.D2A_OSC2MHZEN !== 1'b0)
      begin
        `nnc_error("ATM4", $sformatf("D2A_OSC2MHZEN = %b is not as expectation of 1'b0", `ANA_TOP.D2A_OSC2MHZEN))
      end  
    ...
    // Checking D2A_BIST_SPARE_4 pin
    for (int i=0; i < 100; i++) begin
        force `SOC_TOP.CLKSEL = $random;
        #10000ns;
        rand_bit = `SOC_TOP.CLKSEL;
`ifdef BEHAVIORAL
        //if (`DIG_TOP.u_pinmux.pinmux_if.D2A_ANA_OUT_SEL4 !== rand_bit) begin
        //`nnc_error("ATM4", $sformatf("`DIG_TOP.u_pinmux.D2A_ANA_OUT_SEL4: %b is not as expectation of CLKSEL: %b",`DIG_TOP.u_pinmux.pinmux_if.D2A_ANA_OUT_SEL4, rand_bit))
        //end
`endif
        if (`ANA_TOP.D2A_BIST_SPARE_4 !== rand_bit) begin
        `nnc_error("ATM4", $sformatf("`ANA_TOP.D2A_BIST_SPARE_4: %b is not as expectation of CLKSEL: %b",`ANA_TOP.D2A_BIST_SPARE_4, rand_bit))
        end
        release  `SOC_TOP.CLKSEL;              
    end 
...
`ifdef BEHAVIORAL                             
    ...
    for (int i=0; i < 100; i++) begin
        ...
        `nnc_error("ATM4", $sformatf("`DIG_TOP.u_pinmux.ext_clk : %b is not as expectation of EXT_CLK: %b",`DIG_TOP.u_pinmux.ext_clk, rand_bit))
        end
      release `SOC_TOP.IOBUF_PAD[0];                      
    end  
`endif
    ...
    for (int i=0; i < 100; i++) begin
        ...
        `ifndef POSTLAYOUT_PG
           ...
           `nnc_error("ATM4", $sformatf("`DIG_TOP.u_pinmux.pin_rstn:%b is not as expectation of RESETn: %b",`DIG_TOP.u_pinmux.pin_rstn, rand_bit))
           end
           if (`DIG_TOP.u_pinmux.scan_rst_n !== rand_bit) begin
           `nnc_error("ATM4", $sformatf("`DIG_TOP.u_pinmux.scan_rst_n:%b is not as expectation of RESETn: %b",`DIG_TOP.u_pinmux.scan_rst_n, rand_bit))
           end
           if (`DIG_TOP.u_pinmux.otp_bist_resetn !== rand_bit) begin
           `nnc_error("ATM4", $sformatf("`DIG_TOP.u_pinmux.otp_bist_resetn:%b is not as expectation of RESETn: %b",`DIG_TOP.u_pinmux.otp_bist_resetn, rand_bit))
           end
        `else //in postlayout above all reset signals of pinmux has been optimized so consider only below one
...
`ifdef BEHAVIORAL             
        if (`ANA_WRAPPER_TOP.pinmux_if.D2A_TRIM_SIG[4][7:0]!== {1'b0,rand_num[6:0]}) begin
        `nnc_error("ATM4", $sformatf("`SOC_TOP.pinmux_if.D2A_TRIM_SIG[4][7:0] = %b is not as expectation of = %b", `ANA_WRAPPER_TOP.pinmux_if.D2A_TRIM_SIG[4][7:0], {1'b0,rand_num[6:0]}));
        end
`else
        if (`ANA_WRAPPER_TOP.pinmux_if_D2A_TRIM_SIG[39:32]!== {1'b0,rand_num[6:0]}) begin
        `nnc_error("ATM4", $sformatf("`SOC_TOP.pinmux_if.D2A_TRIM_SIG[39:32] = %b is not as expectation of = %b", `ANA_WRAPPER_TOP.pinmux_if_D2A_TRIM_SIG[39:32], {1'b0,rand_num[6:0]}));
        end
`endif

        if (`ANA_TOP.D2A_VDAC_VTRIM_CH1 !== rand_num[2:0]) begin 
          `nnc_error("ATM4", $sformatf("`ANA_TOP.D2A_VDAC_VTRIM_CH1 = %b is not as expectation of = %b", `ANA_TOP.D2A_VDAC_VTRIM_CH1, rand_num[2:0]));
        end      
  
        if (`ANA_TOP.D2A_CS_TRIM_CH1 !== {1'b0,rand_num[4:3]}) begin
          `nnc_error("ATM4", $sformatf("`ANA_TOP.D2A_CS_TRIM = %b is not as expectation of = %b", `ANA_TOP.D2A_CS_TRIM_CH1, {1'b0, rand_num[4:3]}));
        end

        if (`ANA_TOP.D2A_PUMP_CLK_TRIM_CH1 !== rand_num[5]) begin
          `nnc_error("ATM4", $sformatf("`ANA_TOP.D2A_PUMP_CLK_TRIM_CH1 = %b is not as expectation of = %b", `ANA_TOP.D2A_PUMP_CLK_TRIM_CH1, rand_num[5]));
        end

        if (`ANA_TOP.D2A_PUMP_CLK_TRIM_CH2 !== rand_num[6]) begin
          `nnc_error("ATM4", $sformatf("`ANA_TOP.D2A_PUMP_CLK_TRIM_CH2 = %b is not as expectation of = %b", `ANA_TOP.D2A_PUMP_CLK_TRIM_CH2, rand_num[6]));
        end
