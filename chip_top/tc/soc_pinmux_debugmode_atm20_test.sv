...
// Description	: Testcase soc_pinmux_debugmode_atm20_test                                             
// Designer	: ddang@nanochap.com                                                                 
// Date		: 29-11-1623                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_pinmux_debugmode_atm20_test
`define TESTCFG soc_pinmux_debugmode_atm20_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_pinmux_debugmode_atm20_test_cfg");
    super.new(name);
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_pinmux_debugmode_atm20_test start", NNC_LOW)
...
`endif  
    ...
    if (top_test_cfg.atm !== (29'b1_0000_0000_0000_0000_0000_0000_0000 >> `SOC_TOP.IOBUF_PAD[14:10]))
      begin
        `nnc_error("ATM20", $sformatf("ATM[0:28] = %b is not as expectation of ATM = %b", top_test_cfg.atm, (29'b1_0000_0000_0000_0000_0000_0000_0000 >> `SOC_TOP.IOBUF_PAD[14:10])))
      end  

    //Check default signal  
    if (`ANA_TOP.D2A_BIST_SEL !== 5'b10100)
      begin
        `nnc_error("ATM20", $sformatf("D2A_BIST_SEL = %b is not as expectation of 5'b10100", `ANA_TOP.D2A_BIST_SEL))
      end
   
    if (`ANA_TOP.D2A_NIRS4_EN !== 1'b1)
      begin
        `nnc_error("ATM20", $sformatf("D2A_NIRS4_EN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_NIRS4_EN))
      end
    if (`ANA_TOP.D2A_NIRS_TEST_EN !== 1'b1)
      begin
        `nnc_error("ATM20", $sformatf("D2A_NIRS_TEST_EN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_NIRS_TEST_EN))
      end
    if (`ANA_TOP.D2A_NIRS4_IDAC_EN !== 1'b1)
      begin
        `nnc_error("ATM20", $sformatf("D2A_NIRS4_IDAC_EN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_NIRS4_IDAC_EN))
      end
    if (`ANA_TOP.D2A_BIST_EN !== 1'b1)
      begin
        `nnc_error("ATM20", $sformatf("D2A_BIST_EN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_BIST_EN))
      end  
    if (`ANA_TOP.D2A_OSC8MHZEN !== 1'b1)
      begin
        `nnc_error("ATM20", $sformatf("D2A_OSC8MHZEN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_OSC8MHZEN))
      end 
    if(`ANA_TOP.D2A_BGBUFFER_CPTEST_EN != 1'b1)
      begin
        `nnc_error("ATM20", $sformatf("D2A_BGBUFFER_CPTEST_EN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_BGBUFFER_CPTEST_EN))
      end
...
`ifdef BEHAVIORAL             
        if (`ANA_WRAPPER_TOP.pinmux_if.D2A_ADJ_IO[5][7:0]!== {rand_num[7:0]}) begin
...
`endif
        ...
        `nnc_error("ATM20", $sformatf("{`ANA_TOP.D2A_NIRS4_IDAC_ADJ[7:0]} = %b is not as expectation of = %b", {`ANA_TOP.D2A_NIRS4_IDAC_ADJ[7:0]}, {rand_num[7:0]}));
        end
    ...
    for (int i=0; i < 100; i++) begin
        ...
        `nnc_error("ATM20", $sformatf("`EPROM_TOP.unlock_gpio:%b is not as expectation of CLKSEL: %b",`EPROM_TOP.unlock_gpio, rand_bit))
        end
    ...
    for (int i=0; i < 100; i++) begin
        ...
        `ifndef POSTLAYOUT_PG
           ...
           `nnc_error("ATM20", $sformatf("`RST_CTRL_TOP.scan_rst_n:%b is not as expectation of RESETn: %b",`RST_CTRL_TOP.scan_rst_n, rand_bit))
           end
...
`ifdef BEHAVIORAL                         
    for (int i=0; i < 100; i++) begin
        ...
        `nnc_error("ATM20", $sformatf("`CLK_CTRL_TOP.ext_hfclk : %b is not as expectation of EXT_CLK: %b",`CLK_CTRL_TOP.ext_hfclk, rand_bit))
        end
...
`endif 
    ...
    for (int i=0; i < 100; i++) begin
        ...
        `nnc_error("ATM20", $sformatf("`DIG_TOP.IOBUF_PAD[9] : %b is not as expectation of `DIG_TOP.u_pinmux.i_otp_vpp_en: %b",`SOC_TOP.IOBUF_PAD[9], rand_bit))
        end
