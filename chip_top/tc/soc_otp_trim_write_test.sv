...
//--------------------------------------------------------------------------------------
// File Name	: soc_otp_trim_write_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_otp_trim_write_test                                             
...
// -------------------------------------------------
`define TESTNAME soc_otp_trim_write_test
`define TESTCFG soc_otp_trim_write_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_otp_trim_write_test_cfg");
    super.new(name);
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_otp_trim_write_test start", NNC_LOW)
    ...
    //Trying to read default values of TRIM values from OTP , to make sure match with the table from Reference Manual
     ...
     for(int i=0; i<9 ; i++) begin
       ...
       if(top_test_cfg.rd_data[0] !== i)begin
         `nnc_error("SOC_TEST", $sformatf("SOC_OTP_TRIMS_DBG_SEL_REG %8h !== expected otp_trims_dbg_sel_reg %8h!!!", top_test_cfg.rd_data[0], i))
       end
       else begin
         `nnc_info("SOC_TEST", $sformatf("SOC_OTP_TRIMS_DBG_SEL_REG %8h === expected otp_trims_dbg_sel_reg %8h", top_test_cfg.rd_data[0], i), UVM_LOW)
       end
     ...
     write_rd_trimregs_to_otp(3'd1); //write_rd_trimregs_to_otp(bit wait_for_unlock_clear);
     //8.
    ...
    `WR_BURST_NORMAL_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.data);
     ...
     //2nd time programming       
     `nnc_info("SOC_TEST", " 2nd time Trim Write function for otp", UVM_LOW)
    ...
    `WR_BURST_NORMAL_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.data);
     ...
     //3rd time programming       
     `nnc_info("SOC_TEST", " 2nd time Trim Write function for otp", UVM_LOW)
     //`nnc_info("SOC_TEST", "wait fof fixed delay to complete wr programming", UVM_LOW)
     // To program each location require (1.31*9 +0.1)ms (minimum time require to program one location)
     disable_otp_clk_gating(); 
     write_rd_trimregs_to_otp(3'd2);  //write_rd_trimregs_to_otp(real wr_prgm_delay, bit wait_for_unlock_clear);
     //8.
     read_and_compare_trim_reg_data(top_test_cfg.expected_otp_data, top_test_cfg.save_trim_wdata);
     read_trims_dbg_reg(top_test_cfg.expected_otp_data); 
     //(optional)cross check Analog interface D2A_* TRIM signals 
     ...
     // To program each location require (1.31*9 +0.1)ms (minimum time require to program one location)
     disable_otp_clk_gating(); 
     write_rd_trimregs_to_otp(3'd3);  //write_rd_trimregs_to_otp(real wr_prgm_delay, bit wait_for_unlock_clear);
     //8.
     read_and_compare_trim_reg_data(top_test_cfg.expected_otp_data, top_test_cfg.save_trim_wdata);
     read_trims_dbg_reg(top_test_cfg.expected_otp_data); 
     //(optional)cross check Analog interface D2A_* TRIM signals 
     ...
     // To program each location require (1.31*9 +0.1)ms (minimum time require to program one location)
     disable_otp_clk_gating(); 
     write_rd_trimregs_to_otp(3'd4);  //write_rd_trimregs_to_otp(real wr_prgm_delay, bit wait_for_unlock_clear);
     //8.
     read_and_compare_trim_reg_data(top_test_cfg.expected_otp_data, top_test_cfg.save_trim_wdata); 
     read_trims_dbg_reg(top_test_cfg.expected_otp_data);
     //(optional)cross check Analog interface D2A_* TRIM signals 
...
//     #1us;
    ...
    // --------------------------------------------------------
    #10000ns;
    `nnc_info("SOC_TEST", "soc_otp_trim_write_test end now", NNC_LOW)
  ...
  task write_rd_trimregs_to_otp(bit[2:0] wait_for_unlock_clear);
    ...
    //1.set valid tag
    set_valid_tag();    //for successfull reload after poweroff==>power on set valid tag 
...
//    end
      ...
      if(wait_for_unlock_clear === 3'd0) begin
         `nnc_info("SOC_TEST", "wait for", UVM_LOW)
    ...
    //7.wait reload done
    wait_reload_done();
    `nnc_info("SOC_TEST", $sformatf("reload_done==1!!!"), UVM_LOW)

    //(optional check)just to corss check by disabling otp_clk
...
task read_and_compare_trim_reg_data(logic [7:0] prev_wdata_1[11], logic [7:0] cur_wdata_1[11]);
    ...
    for (int i =0; i<8; i++)begin
        ...
        //     `nnc_info("SOC_TEST", $sformatf("0==>1 detection, for-loop i=%h expected_wdata=%8h ", i, top_test_cfg.expected_otp_data[i]), UVM_LOW) 
 
        //end else if ((top_test_cfg.prev_wdata[i] === 1'b1) && (top_test_cfg.save_trim_wdata[i] === 1'b0))begin   // 1==> 0
        //     top_test_cfg.expected_otp_data[i] = top_test_cfg.prev_wdata[i]; //1'b1;
        //     `nnc_info("SOC_TEST", $sformatf("1==>0 dectection, for-loop i=%h expected_wdata=%8h ", i, top_test_cfg.expected_otp_data[i]), UVM_LOW) 

        //end else begin
        //    top_test_cfg.expected_otp_data[i] = top_test_cfg.save_trim_wdata[i]; // 0==>0, 1==>1
        //end
        top_test_cfg.expected_otp_data[i] = (prev_wdata_1[i] | cur_wdata_1[i]);
        `nnc_info("SOC_TEST", $sformatf("expected data=%8h ", top_test_cfg.expected_otp_data[i]), UVM_LOW)                
    end 
 ...
 task wait_wr_working_high();
    ...
    `nnc_info("SOC_TEST", $sformatf("Wait for wr_wroking as HIGH"), UVM_LOW)
     do begin
        assert(top_test_cfg.randomize() with { reg_addr == `SOC_OTP_DEBUG_1_REG; no_of_bytes == 1;});
        `RD_BURST_NORMAL_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.rd_data);
        `nnc_info("SOC_TEST", $sformatf("wr_working %h !!!", top_test_cfg.rd_data[0][6]), UVM_LOW) 
