...
//--------------------------------------------------------------------------------------
// File Name	: soc_wavegen_drva_sawtooth_singlewave_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_wavegen_drva_sawtooth_singlewave_test                                             
...
//***************************************************************************************
// NOTE : The test is intented  to generate triangle wave for driver1 & driver2. But 
...
// -------------------------------------------------
`define TESTNAME soc_wavegen_drva_sawtooth_singlewave_test
`define TESTCFG soc_wavegen_drva_sawtooth_singlewave_test_cfg
...
class `TESTCFG extends soc_wavegen_base_test_cfg;
  ...
  function new (string name = "soc_wavegen_drva_sawtooth_singlewave_test_cfg");
    super.new(name);
...
class `TESTNAME extends soc_wavegen_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_wavegen_drva_sawtooth_singlewave_test start", NNC_LOW)
    ...
    for (int i=0; i < `DUT_IF.no_of_point_a; i++) begin
      `DUT_IF.hex_data_a[i] = top_test_cfg.sawtooth_data[i]; // expected hex values
      `DUT_IF.hex_data_b[i] = top_test_cfg.sawtooth_data[i]; // expected hex values
    end
    ...
    if(top_test_cfg.PRELOAD === 2'b10)
    	`nnc_info("SOC_TEST", "Enable driver0 using control register with preloaded triangular values", NNC_LOW)
    ...
    if(top_test_cfg.PRELOAD === 2'b10)
    	`nnc_info("SOC_TEST", "Enable driver1 using control register with preloaded triangular values", NNC_LOW)
    else if(top_test_cfg.PRELOAD === 2'b11)
    ...
    // --------------------------------------------------------
    #200ms;
    `nnc_info("SOC_TEST", "soc_wavegen_drva_sawtooth_singlewave_test end now", NNC_LOW)
