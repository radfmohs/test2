...
//--------------------------------------------------------------------------------------
// File Name	: soc_anac_short_pulsewave_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_anac_short_pulsewave_test                                             
...
//*******************************************************************************************
// NOTE : This test is used to check short detection using pulse waveforms.
...
//        counter_TH = 10 * PW, where PW is the pulse width of either positive half or 
...
// -------------------------------------------------
`define TESTNAME soc_anac_short_pulsewave_test
`define TESTCFG soc_anac_short_pulsewave_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_anac_short_pulsewave_test_cfg");
    super.new(name);
  ...
  //waveform_sel
  constraint c_waveform_sel    { (preload_sel == 2'b01) -> waveform_sel inside {[0:0]};//do not generate arbitrary wave
  ...
  constraint c_PULLAB_pos_en    { (rest_en == 0 && silent_en == 0) -> PULLAB_pos_en == 0; }
  constraint c_PULLAB_neg_en    { (rest_en == 0 && silent_en == 0) -> PULLAB_neg_en == 0; }
  ...
  constraint c_cnt_percent_of_timer_TH1 { ((neg_ena == 1'b0) || (pos_dis == 1'b1)) -> cnt_percent_of_timer_TH1 == 40;//DC wave
                                          ((neg_ena == 1'b1) && (pos_dis == 1'b0)) -> cnt_percent_of_timer_TH1 == 40;//continous square wave(no gap)
                                        }
  ...
  constraint c_cnt_percent_of_timer_TH2 { ((neg_ena == 1'b0) || (pos_dis == 1'b1)) -> cnt_percent_of_timer_TH2 == 40;//DC wave
                                          ((neg_ena == 1'b1) && (pos_dis == 1'b0)) -> cnt_percent_of_timer_TH2 == 40;//continous square wave(no gap)
                                        }
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_anac_short_pulsewave_test start", NNC_LOW)
    ...
    // --------------------------------------------------------
    #10000ns;
    `nnc_info("SOC_TEST", "soc_anac_short_pulsewave_test end now", NNC_LOW)
  ...
  begin
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_CTRL0_REG + WG_BASE); (`DUT_IF.preload_sel == 2'b01) -> top_test_cfg.dac_bit_len_sel == 1'b0; wr_data[0] == {2'b0, top_test_cfg.dac_bit_len_sel,top_test_cfg.auto_man, 4'b0};});
...
//    if((`DUT_IF.rest_en == 1) || (`DUT_IF.silent_en == 1)) begin//there is interphase gap
	//top_test_cfg.short_CH1_counter_th = ((`DUT_IF.no_of_cycles_CH1 + `DUT_IF.no_of_waveforms)/(`DUT_IF.no_of_waveforms+1) * top_test_cfg.pulse_width_ch1) - 1;
	//top_test_cfg.short_CH2_counter_th = ((`DUT_IF.no_of_cycles_CH2 + `DUT_IF.no_of_waveforms)/(`DUT_IF.no_of_waveforms+1) * top_test_cfg.pulse_width_ch2) - 1;
      // 2 extra clks considered , as counter resp starts 2 clk later than source signal
	top_test_cfg.short_CH1_counter_th = ((`DUT_IF.no_of_cycles_CH1 + `DUT_IF.no_of_waveforms)/(`DUT_IF.no_of_waveforms+1) * top_test_cfg.pulse_width_ch1) - 3;
	top_test_cfg.short_CH2_counter_th = ((`DUT_IF.no_of_cycles_CH2 + `DUT_IF.no_of_waveforms)/(`DUT_IF.no_of_waveforms+1) * top_test_cfg.pulse_width_ch2) - 3;
//    end
//    else begin
