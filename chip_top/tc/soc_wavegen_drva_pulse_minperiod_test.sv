...
//--------------------------------------------------------------------------------------
// File Name	: soc_wavegen_drva_pulse_minperiod_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_wavegen_drva_pulse_minperiod_test                                             
...
//***************************************************************************************
// NOTE : This test uses minimum period settings to generate pulse waveform
//***************************************************************************************

// =================================================
// Testcase name is defined
// -------------------------------------------------
`define TESTNAME soc_wavegen_drva_pulse_minperiod_test
`define TESTCFG soc_wavegen_drva_pulse_minperiod_test_cfg
...
class `TESTCFG extends soc_wavegen_base_test_cfg;
  ...
  function new (string name = "soc_wavegen_drva_pulse_minperiod_test_cfg");
    super.new(name);
  ...
  //points_sel
  constraint c_points_sel      { ((waveform_sel == 3'b000) && (neg_ena == 1'b1) && (pos_dis == 1'b0) && (pos_neg_diff_sel == 1'b0)) -> points_sel inside {[6:6]};//1 point
                                 ((waveform_sel == 3'b000) && (neg_ena == 1'b1) && (pos_dis == 1'b0) && (pos_neg_diff_sel == 1'b1)) -> points_sel inside {[6:6]};//1 point
...
class `TESTNAME extends soc_wavegen_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_wavegen_drva_pulse_minperiod_test start", NNC_LOW)
    ...
    `DUT_IF.wg_hlf_wave0_lim[i] = 32'h00000001;//top_test_cfg.hlf_wave_lim / top_test_cfg.NO_OF_POINTS;
    `DUT_IF.wg_neg_hlf_wave0_lim[i] = 32'h00000001;//top_test_cfg.neg_hlf_wave_lim / top_test_cfg.NO_OF_POINTS;
    `DUT_IF.wg_rest_wave0_lim[i] = 16'h0;//top_test_cfg.rest_lim;
    `DUT_IF.wg_silent_wave0_lim[i] = 32'h0;//top_test_cfg.silent_lim;
    `nnc_info("SOC_TEST", $sformatf("******** Driver (%d) WAVE 0 ******** CLK_FREQ: %dKhz, HALF_PERIOD_CLKS_PER_POINT: %d", i, top_test_cfg.clk_freq, `DUT_IF.wg_hlf_wave0_lim[i]), NNC_LOW)

    //wavegen_calc_clock_num(clk_freq (KHz), rest_t (us), silent_t (us), hlf_wave_per (us), neg_hlf_wave_per (us))
    //wavegen_calc_clock_num(top_test_cfg.clk_freq, 100, 1000, top_test_cfg.half_period1, top_test_cfg.half_period1);
    `DUT_IF.wg_hlf_wave1_lim[i] = 32'h00000001;//top_test_cfg.hlf_wave_lim / top_test_cfg.NO_OF_POINTS;
    `DUT_IF.wg_neg_hlf_wave1_lim[i] = 32'h00000001;//top_test_cfg.neg_hlf_wave_lim / top_test_cfg.NO_OF_POINTS;
    `DUT_IF.wg_rest_wave1_lim[i] = 16'h0;//top_test_cfg.rest_lim;
    `DUT_IF.wg_silent_wave1_lim[i] = 32'h0;//top_test_cfg.silent_lim;
    `nnc_info("SOC_TEST", $sformatf("******** Driver (%d) WAVE 1 ******** CLK_FREQ: %dKhz, HALF_PERIOD_CLKS_PER_POINT: %d", i, top_test_cfg.clk_freq, `DUT_IF.wg_hlf_wave1_lim[i]), NNC_LOW)

    //wavegen_calc_clock_num(clk_freq (KHz), rest_t (us), silent_t (us), hlf_wave_per (us), neg_hlf_wave_per (us))
    //wavegen_calc_clock_num(top_test_cfg.clk_freq, 255, 8000, top_test_cfg.half_period2, top_test_cfg.half_period2);
    `DUT_IF.wg_hlf_wave2_lim[i] = 32'h00000001;//top_test_cfg.hlf_wave_lim / top_test_cfg.NO_OF_POINTS;
    `DUT_IF.wg_neg_hlf_wave2_lim[i] = 32'h00000001;//top_test_cfg.neg_hlf_wave_lim / top_test_cfg.NO_OF_POINTS;
    `DUT_IF.wg_rest_wave2_lim[i] = 16'h0;//top_test_cfg.rest_lim;
    `DUT_IF.wg_silent_wave2_lim[i] = 32'h0;//top_test_cfg.silent_lim;
    `nnc_info("SOC_TEST", $sformatf("******** Driver (%d) WAVE 2 ******** CLK_FREQ: %dKhz, HALF_PERIOD_CLKS_PER_POINT: %d", i, top_test_cfg.clk_freq, `DUT_IF.wg_hlf_wave2_lim[i]), NNC_LOW)
    end
    ...
    // --------------------------------------------------------
    #50ms;
    `nnc_info("SOC_TEST", "soc_wavegen_drva_pulse_minperiod_test end now", NNC_LOW)
  ...
  begin
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_CONFIG_REG0 + WG_BASE); wr_data[0] == {top_test_cfg.POS_OFF, 1'b1, 1'b0, 1'b0, 1'b1, 1'b1, top_test_cfg.NEG_ON, 1'b1};});
