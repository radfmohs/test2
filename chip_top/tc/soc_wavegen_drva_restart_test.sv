...
//--------------------------------------------------------------------------------------
// File Name	: soc_wavegen_drva_restart_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_wavegen_drva_restart_test                                             
...
//***************************************************************************************
// NOTE : This test intention is to check restarting behaviour of wavegen after disabling
//***************************************************************************************

// =================================================
// Testcase name is defined
// -------------------------------------------------
`define TESTNAME soc_wavegen_drva_restart_test
`define TESTCFG soc_wavegen_drva_restart_test_cfg
...
class `TESTCFG extends soc_wavegen_base_test_cfg;
  ...
  logic [15:0]     rest_wave0_lim; // number of clocks for each rest period wave0
  logic [31:0]     silent_wave0_lim; // number of clocks for each silent period wave0
  logic [15:0]     rest_wave1_lim; // number of clocks for each rest period wave1
  logic [31:0]     silent_wave1_lim; // number of clocks for each silent period wave1
  logic [15:0]     rest_wave2_lim; // number of clocks for each rest period wave2
  logic [31:0]     silent_wave2_lim; // number of clocks for each silent period wave2
  ...
  function new (string name = "soc_wavegen_drva_restart_test_cfg");
    super.new(name);
  ...
  //rest_en
  constraint c_rest_en         { rest_en == 1'b1;}

  //rest_en
  constraint c_silent_en       { silent_en == 1'b1;}

  //points_sel
  constraint c_points_sel      { ((waveform_sel == 3'b000) && (neg_ena == 1'b1) && (pos_dis == 1'b0) && (pos_neg_diff_sel == 1'b0)) -> points_sel inside {[0:6]};
                                 ((waveform_sel == 3'b000) && (neg_ena == 1'b1) && (pos_dis == 1'b0) && (pos_neg_diff_sel == 1'b1)) -> points_sel inside {[7:7]};
  ...
  //load_points_sel
  constraint c_load_points_sel { (((waveform_sel == 3'b001) || (waveform_sel == 3'b010)) && (preload_sel == 2'b11)) -> load_points_sel == 1'b1; (preload_sel == 2'b01) -> load_points_sel == 1'b0;}
  ...
  //PULLAB_lim
  constraint c_PULLAB_lim      { PULLAB_lim == 6'b111111;}
...
class `TESTNAME extends soc_wavegen_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_wavegen_drva_restart_test start", NNC_LOW)
    ...
    // --------------------------------------------------------
    top_test_cfg.PULLAB_pos_en = 1'b1;
    top_test_cfg.PULLAB_neg_en = 1'b1;
    assert(top_test_cfg.randomize() with {reg_addr == `SOC_ADDR_WG_DRV_PULLBA_REG; wr_data[0] == {top_test_cfg.PULLAB_pos_en, top_test_cfg.PULLAB_neg_en, top_test_cfg.PULLAB_lim};});
    `nnc_info("SOC_TEST", "Set pullab reg", NNC_LOW)
    `WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_PULLBA_REG+`WAVEGEN_1_ADDR_BASE); wr_data[0] == {top_test_cfg.PULLAB_pos_en, top_test_cfg.PULLAB_neg_en, top_test_cfg.PULLAB_lim};});
    `nnc_info("SOC_TEST", "Set pullab reg", NNC_LOW)
    `WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);
    ...
    // --------------------------------------------------------
    #50ms;
    `nnc_info("SOC_TEST", "soc_wavegen_drva_restart_test end now", NNC_LOW)
  ...
  begin
    ...
    wavegen_calc_clock_num(top_test_cfg.clk_freq, 100, 8000, top_test_cfg.half_period0, top_test_cfg.half_period0);
    ...
    top_test_cfg.rest_wave0_lim = 16'hFF;//top_test_cfg.rest_lim;
    top_test_cfg.silent_wave0_lim = top_test_cfg.silent_lim;
    ...
    //wavegen_calc_clock_num(clk_freq (KHz), rest_t (us), silent_t (us), hlf_wave_per (us), neg_hlf_wave_per (us))
    wavegen_calc_clock_num(top_test_cfg.clk_freq, 150, 4000, top_test_cfg.half_period1, top_test_cfg.half_period1);
    ...
    top_test_cfg.rest_wave1_lim = top_test_cfg.rest_lim;
    top_test_cfg.silent_wave1_lim = top_test_cfg.silent_lim;
    ...
    //wavegen_calc_clock_num(clk_freq (KHz), rest_t (us), silent_t (us), hlf_wave_per (us), neg_hlf_wave_per (us))
    wavegen_calc_clock_num(top_test_cfg.clk_freq, 250, 8000, top_test_cfg.half_period2, top_test_cfg.half_period2);
    ...
    top_test_cfg.rest_wave2_lim = top_test_cfg.rest_lim;
    top_test_cfg.silent_wave2_lim = top_test_cfg.silent_lim;
  ...
  begin
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_CTRL0_REG + WG_BASE); (top_test_cfg.PRELOAD == 2'b01) -> top_test_cfg.dac_bit_len_sel == 1'b0; wr_data[0] == {2'b0, top_test_cfg.dac_bit_len_sel,top_test_cfg.auto_man, 4'b0};});
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_REST_T_REG01 + WG_BASE); no_of_bytes == 2;  wr_data[0] == top_test_cfg.rest_wave0_lim[15:8]; wr_data[1] == top_test_cfg.rest_wave0_lim[7:0];});
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_SILENT_T_REG01 + WG_BASE); no_of_bytes == 3; wr_data[0] == top_test_cfg.silent_wave0_lim[23:16]; wr_data[1] == top_test_cfg.silent_wave0_lim[15:8]; wr_data[2] == top_test_cfg.silent_wave0_lim[7:0];});
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_REST_CLK1_REG01 + WG_BASE); no_of_bytes == 2;  wr_data[0] == top_test_cfg.rest_wave1_lim[15:8]; wr_data[1] == top_test_cfg.rest_wave1_lim[7:0];});
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_SILENT_CLK1_REG01 + WG_BASE); no_of_bytes == 3; wr_data[0] == top_test_cfg.silent_wave1_lim[23:16]; wr_data[1] == top_test_cfg.silent_wave1_lim[15:8]; wr_data[2] == top_test_cfg.silent_wave1_lim[7:0];});
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_REST_CLK2_REG01 + WG_BASE); no_of_bytes == 2;  wr_data[0] == top_test_cfg.rest_wave2_lim[15:8]; wr_data[1] == top_test_cfg.rest_wave2_lim[7:0];});
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_SILENT_CLK2_REG01 + WG_BASE); no_of_bytes == 3; wr_data[0] == top_test_cfg.silent_wave2_lim[23:16]; wr_data[1] == top_test_cfg.silent_wave2_lim[15:8]; wr_data[2] == top_test_cfg.silent_wave2_lim[7:0];});
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_CONFIG_REG0 + WG_BASE); wr_data[0] == {top_test_cfg.POS_OFF, 1'b1, 1'b0, 1'b0, 1'b1, top_test_cfg.silent_en, top_test_cfg.NEG_ON, top_test_cfg.rest_en};});
    ...
    // Write to ADDR_WG_DRV_PULLBA_REG
    // --------------------------------------------------------
    top_test_cfg.PULLAB_pos_en = 1'b0;
    top_test_cfg.PULLAB_neg_en = 1'b0;
