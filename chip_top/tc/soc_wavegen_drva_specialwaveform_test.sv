...
//--------------------------------------------------------------------------------------
// File Name	: soc_wavegen_drva_specialwaveform_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_wavegen_drva_specialwaveform_test                                             
...
//********************************************************************************************
// NOTE : This test follows the same configuration sequence as basic sine/pulse/triangle test, 
//        but the intention of this test is to generate special waveform of silent time after 
//        few no: of cycles.
//********************************************************************************************
...
// -------------------------------------------------
`define TESTNAME soc_wavegen_drva_specialwaveform_test
`define TESTCFG soc_wavegen_drva_specialwaveform_test_cfg
...
class `TESTCFG extends soc_wavegen_base_test_cfg;
  ...
  rand logic       special_silent_period_ctrl;
  rand logic [15:0] special_silent_period;
       logic [1:0] PRELOAD;
       logic       LOAD_POINTS;
  ...
  function new (string name = "soc_wavegen_drva_specialwaveform_test_cfg");
    super.new(name);
  ...
  //silent_en
  constraint c_silent_en       { silent_en == 1;}

  //special_silent_period_ctrl
  constraint c_special_silent_period_ctrl { special_silent_period_ctrl == 1;}

  //special_silent_period
  constraint c_special_silent_period { special_silent_period inside {[0:100]};}
  // -----------------------------------------------
...
class `TESTNAME extends soc_wavegen_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_wavegen_drva_specialwaveform_test start", NNC_LOW)
    ...
    `nnc_info("SOC_TEST", $sformatf("NO_OF_WAVEFORMS: %d, POS_OFF: %d, NEG_ON:%d", top_test_cfg.NO_OF_WAVEFORMS, top_test_cfg.POS_OFF, top_test_cfg.NEG_ON), NNC_LOW)
    ...
    assert(top_test_cfg.randomize() with {half_period0 == (half_period_limit+1); half_period1 > half_period_limit; half_period2 > half_period_limit;});
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == `SOC_ADDR_WG_DRV_CTRL0_REG; wr_data[0] == {2'b0, dac_bit_len_sel,auto_man, 4'b0};});
    `WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);

    // --------------------------------------------------------
    // Write to SOC_ADDR_WG_DRV_CTRL0_REG for drv1
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_CTRL0_REG + `WAVEGEN_1_ADDR_BASE); wr_data[0] == {2'b0, dac_bit_len_sel,auto_man, 4'b0};});
    `WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);

    // ------------------------------------------------------------
    // Write burst starting from SOC_ADDR_WG_DRV_CTRL1_REG for drv0
    // ------------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == `SOC_ADDR_WG_DRV_CTRL1_REG; no_of_bytes == 2; wr_data[0] == {1'b0, dac0_msb_sel, dac0_data_h}; wr_data[1] == dac0_data_l;});
    ...
    // ------------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_CTRL1_REG + `WAVEGEN_1_ADDR_BASE); no_of_bytes == 2; wr_data[0] == {1'b0, dac1_msb_sel, dac1_data_h}; wr_data[1] == dac1_data_l;});
    `WR_BURST_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.wr_data);

    // ----------------------------------------------------------------
    // Write burst starting from SOC_NO_OF_NUM_SILENT_CTR0_REG for drv0
    // ----------------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == `SOC_NO_OF_NUM_SILENT_CTR0_REG; no_of_bytes == 3;wr_data[0] == special_silent_period[15:8];wr_data[1] == special_silent_period[7:0];wr_data[2] == {7'h0,special_silent_period_ctrl};});
    `WR_BURST_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.wr_data);
    `nnc_info("SOC_TEST", $sformatf("Special Silent period of dac0:%h", top_test_cfg.special_silent_period), NNC_LOW)
    // ----------------------------------------------------------------
    // Write burst starting from SOC_NO_OF_NUM_SILENT_CTR0_REG for drv1
    // ----------------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_NO_OF_NUM_SILENT_CTR0_REG + `WAVEGEN_1_ADDR_BASE); no_of_bytes == 3;wr_data[0] == special_silent_period[15:8];wr_data[1] == special_silent_period[7:0];wr_data[2] == {7'h0,special_silent_period_ctrl};});
    `WR_BURST_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.wr_data);
    `nnc_info("SOC_TEST", $sformatf("Special Silent period of dac1:%h", top_test_cfg.special_silent_period), NNC_LOW)
    ...
    // --------------------------------------------------------
    #50ms;
    `nnc_info("SOC_TEST", "soc_wavegen_drva_specialwaveform_test end now", NNC_LOW)
  ...
  begin
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_CONFIG_REG0 + WG_BASE); (`DUT_IF.wg_drv_sel == 1) -> multielectrode_en == 1; wr_data[0] == {POS_OFF, multielectrode_en, 1'b0, 1'b0, sourceB_en, silent_en, NEG_ON, rest_en};});
	...
	// --------------------------------------------------------
    	...
    	// --------------------------------------------------------
	assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_IN_WAVE_REG01 + WG_BASE); wr_data[0] == sine_data[i][7:0];});
    	`WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);
    end
    end
    // --------------------------------------------------------
