...
//--------------------------------------------------------------------------------------
// File Name	: soc_wavegen_drva_emssine_scaling_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_wavegen_drva_emssine_scaling_test                                             
...
//******************************************************************************************
// NOTE : This test follows ems sine test. But in this test, upon interrupt no need to load 
//        the new arbitrary wave data. Instead the different values are accomplished by 
//        updating the scaling register, which saves the no: of spi access
//******************************************************************************************

// =================================================
// Testcase name is defined
// -------------------------------------------------
`define TESTNAME soc_wavegen_drva_emssine_scaling_test
`define TESTCFG soc_wavegen_drva_emssine_scaling_test_cfg
...
class `TESTCFG extends soc_wavegen_base_test_cfg;
  ...
  function new (string name = "soc_wavegen_drva_emssine_scaling_test_cfg");
    super.new(name);
...
*/
  ...
  //preload_sel
  constraint c_preload_sel     { preload_sel inside {2,2};}
...
class `TESTNAME extends soc_wavegen_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_wavegen_drva_emssine_scaling_test start", NNC_LOW)
    ...
    // --------------------------------------------------------
    // Write to SOC_ADDR_WG_DRV_CTRL2_REG for drv0
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == `SOC_ADDR_WG_DRV_CTRL2_REG; wr_data[0] == 8'h80;});//set MULTI_ARGO_CONTROL = 1
    `WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);

    // --------------------------------------------------------
    // Write to SOC_ADDR_WG_DRV_CTRL2_REG for drv1
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_CTRL2_REG + `WAVEGEN_1_ADDR_BASE); wr_data[0] == 8'h80;});//set MULTI_ARGO_CONTROL = 1
    `WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);
    ...
    // --------------------------------------------------------
    #10000ns;
    `nnc_info("SOC_TEST", "soc_wavegen_drva_emssine_scaling_test end now", NNC_LOW)
  ...
  begin
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_NEG_SCALE_REG0 + WG_BASE); wr_data[0] == 8'h02;});
    `nnc_info("SOC_TEST", "Set negative scale value", NNC_LOW)
    `WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);

    // --------------------------------------------------------
    // Write to ADDR_WG_DRV_POS_SCALE_REG0 (By default it is 1)
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_POS_SCALE_REG0 + WG_BASE); wr_data[0] == 8'h02;});
    `nnc_info("SOC_TEST", "Set positive scale value", NNC_LOW)
    `WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_INT_REG01 + WG_BASE); no_of_bytes == 3; wr_data[0] == 8'h3f; wr_data[1] == 8'h00; wr_data[2] == 8'h01;});
  ...
  task wavegen_drv_int_update;
    int s = 0;
    int c = 0;
    logic [6:0] scale_val = 7'h2;
    logic scale_up_down = 0;
`ifdef BEHAVIORAL
    int hold_cnt = 5;
    int max_scale = 16;
`else
...
`endif
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_INT_NUM_REG + (`WAVEGEN_1_ADDR_BASE * `DUT_IF.wg_drv_sel)); wr_data[0] == 8'h01;});
    `WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);
	  ...
	  if ((top_test_cfg.rd_data[2][3]===1'b1) & (top_test_cfg.rd_data[2][2:0]===`DUT_IF.wg_drv_sel)) begin //also check if interrupt in this block is enabled (bit 8 is active)
		...
		if(top_test_cfg.rd_data[2][4]===1) begin
		    ...
		    if (top_test_cfg.rd_data[1] === 8'h00) begin //first address is at the first half of the wave
				`nnc_info("SOC_TEST", $sformatf("Update the new wave data scale_val: %d, scale_up_down: %d", scale_val, scale_up_down), NNC_LOW)
				if((s===0) && (scale_val>0)) begin
				    `nnc_info("SOC_TEST", "Ramp up!!!", NNC_LOW)
				    if(scale_val === (max_scale/2)) begin
					scale_val = max_scale;
				        scale_up_down = 0;
				    	s = 1;
				    end
				...
				// --------------------------------------------------------
    				// Write to ADDR_WG_DRV_NEG_SCALE_REG0 (By default it is 1)
    				// --------------------------------------------------------
    				assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_NEG_SCALE_REG0 + (`WAVEGEN_1_ADDR_BASE * `DUT_IF.wg_drv_sel)); wr_data[0] == {scale_up_down, scale_val};});
				...
				// --------------------------------------------------------
    				// Write to ADDR_WG_DRV_POS_SCALE_REG0 (By default it is 1)
    				// --------------------------------------------------------
    				assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_POS_SCALE_REG0 + (`WAVEGEN_1_ADDR_BASE * `DUT_IF.wg_drv_sel)); wr_data[0] == {scale_up_down, scale_val};});
