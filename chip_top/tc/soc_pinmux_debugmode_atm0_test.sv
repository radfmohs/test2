...
//--------------------------------------------------------------------------------------
// File Name	: soc_pinmux_debugmode_atm0_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_pinmux_debugmode_atm0_test                                             
// Designer	: ddang@nanochap.com                                                                 
// Date		: 29-11-1623                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_pinmux_debugmode_atm0_test
`define TESTCFG soc_pinmux_debugmode_atm0_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  // -----------------------------------------------
  rand logic [7:1] data[256];
  rand int         no_of_bytes; 
  rand logic [6:1] reg_addr;
  rand logic [6:1] cmd;  
  logic [7:1] read_data[];
  logic [29:0] atm;
  ...
  function new (string name = "soc_pinmux_debugmode_atm0_test_cfg");
    super.new(name);
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_pinmux_debugmode_atm0_test start", NNC_LOW)
...
`ifndef FPGA
   ...
   // ------------------------------------------------------ 
   // Part II: Checking ATM0 will be asserted in interface
   // ------------------------------------------------------
    #100000ns;
`ifdef BEHAVIORAL    
...
`endif
    if (top_test_cfg.atm !== (30'b10_0000_0000_0000_0000_0000_0000_0000 >> `SOC_TOP.IOBUF_PAD[14:10]))
      begin
        `nnc_error("ATM0", $sformatf("ATM[0:29] = %b is not as expectation of ATM = %b", top_test_cfg.atm, (30'b10_0000_0000_0000_0000_0000_0000_0000 >> `SOC_TOP.IOBUF_PAD[14:10])))
      end 
    ...
    if (`ANA_TOP.D2A_BIST_EN !== 1'b1)
      begin
        `nnc_error("ATM0", $sformatf("D2A_BIST_EN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_BIST_EN))
      end  
    if (`ANA_TOP.D2A_OSC8MHZEN !== 1'b1)
      begin
        `nnc_error("ATM0", $sformatf("D2A_OSC8MHZEN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_OSC8MHZEN))
      end 
    if(`ANA_TOP.D2A_BGBUFFER_CPTEST_EN != 1'b1)
      begin
        `nnc_error("ATM0", $sformatf("D2A_BGBUFFER_CPTEST_EN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_BGBUFFER_CPTEST_EN))
      end
...
`ifdef BEHAVIORAL             
        if (`ANA_WRAPPER_TOP.pinmux_if.D2A_TRIM_SIG[0][7:0]!== {rand_num[7:0]}) begin
        `nnc_error("ATM0", $sformatf("`SOC_TOP.pinmux_if.D2A_TRIM_SIG[0][7:0] = %b is not as expectation of = %b", `ANA_WRAPPER_TOP.pinmux_if.D2A_TRIM_SIG[0][7:0], {rand_num[7:0]}));
        end
`else
        if (`ANA_WRAPPER_TOP.pinmux_if_D2A_TRIM_SIG[7:0]!== {rand_num[7:0]}) begin
        `nnc_error("ATM0", $sformatf("`SOC_TOP.pinmux_if.D2A_TRIM_SIG[7:0] = %b is not as expectation of = %b", `ANA_WRAPPER_TOP.pinmux_if_D2A_TRIM_SIG[7:0], {rand_num[7:0]}));
        end
`endif
        if (`ANA_TOP.D2A_BG_TRIM!== {rand_num[7:0]}) begin
        `nnc_error("ATM0", $sformatf("`ANA_TOP.D2A_BG_TRIM = %b is not as expectation of = %b", `ANA_TOP.D2A_BG_TRIM, {rand_num[7:0]}));
        end
    ...
    for (int i=0; i < 100; i++) begin
        ...
        `nnc_error("ATM0", $sformatf("`EPROM_TOP.unlock_gpio:%b is not as expectation of CLKSEL: %b",`EPROM_TOP.unlock_gpio, rand_bit))
        end
    ...
    for (int i=0; i < 100; i++) begin
        ...
        `ifndef POSTLAYOUT_PG
           ...
           `nnc_error("ATM0", $sformatf("`RST_CTRL_TOP.ext_resetn:%b is not as expectation of RESETn: %b",`RST_CTRL_TOP.ext_resetn, rand_bit))
           end
           if (`RST_CTRL_TOP.scan_rst_n !== rand_bit) begin
           `nnc_error("ATM0", $sformatf("`RST_CTRL_TOP.scan_rst_n:%b is not as expectation of RESETn: %b",`RST_CTRL_TOP.scan_rst_n, rand_bit))
           end
           if (`RST_CTRL_TOP.otp_bist_resetn !== rand_bit) begin
           `nnc_error("ATM0", $sformatf("`RST_CTRL_TOP.otp_bist_resetn:%b is not as expectation of RESETn: %b",`RST_CTRL_TOP.otp_bist_resetn, rand_bit))
           end
...
`ifdef BEHAVIORAL                         
    for (int i=0; i < 100; i++) begin
        ...
        `nnc_error("ATM0", $sformatf("`CLK_CTRL_TOP.ext_hfclk : %b is not as expectation of EXT_CLK: %b",`CLK_CTRL_TOP.ext_hfclk, rand_bit))
        end
...
`endif
    ...
    for (int i=0; i < 100; i++) begin
        ...
        `nnc_error("ATM0", $sformatf("`DIG_TOP.IOBUF_PAD[9] : %b is not as expectation of `DIG_TOP.u_pinmux.i_otp_vpp_en: %b",`SOC_TOP.IOBUF_PAD[9], rand_bit))
        end
...
/*
    ...
    // ---------------------------------------------------------
    //Check default signal           
    if (`ANA_TOP.D2A_BIST_SEL !== 4'b0000)
      begin
        `nnc_error("ATM0", $sformatf("D2A_BIST_SEL = %b is not as expectation of 4'b0000", `ANA_TOP.D2A_BIST_SEL))
      end  
    if (`ANA_TOP.D2A_BIST_EN !== 1'b1)
    ...
    if (`ANA_TOP.D2A_OSC2MHZEN !== 1'b0)
      begin
        `nnc_error("ATM0", $sformatf("D2A_OSC2MHZEN = %b is not as expectation of 1'b0", `ANA_TOP.D2A_OSC2MHZEN))
      end  
    ...
    // Checking ATM
    for (int i=0; i < 100; i++) begin
        force {`SOC_TOP.IOBUF_PAD[6:1]} = $random;
        #10000ns;
        rand_num[5:0] = `SOC_TOP.IOBUF_PAD[6:1]; 
`ifdef BEHAVIORAL             
        if (`ANA_WRAPPER_TOP.pinmux_if.D2A_TRIM_SIG[0][7:0]!== {2'b0,rand_num[5:0]}) begin
        `nnc_error("ATM0", $sformatf("`SOC_TOP.pinmux_if.D2A_TRIM_SIG[0][7:0] = %b is not as expectation of = %b", `ANA_WRAPPER_TOP.pinmux_if.D2A_TRIM_SIG[0][7:0], {2'b0,rand_num[5:0]}));
        end
`else
        if (`ANA_WRAPPER_TOP.pinmux_if_D2A_TRIM_SIG[7:0]!== {2'b0,rand_num[5:0]}) begin
        `nnc_error("ATM0", $sformatf("`SOC_TOP.pinmux_if.D2A_TRIM_SIG[7:0] = %b is not as expectation of = %b", `ANA_WRAPPER_TOP.pinmux_if_D2A_TRIM_SIG[7:0], {2'b0,rand_num[5:0]}));
        end
`endif
        if (`ANA_TOP.D2A_BG_TRIM!== {2'b0,rand_num[5:0]}) begin
        `nnc_error("ATM0", $sformatf("`ANA_TOP.D2A_BG_TRIM = %b is not as expectation of = %b", `ANA_TOP.D2A_BG_TRIM, {2'b0,rand_num[5:0]}));
        end

        release  `SOC_TOP.IOBUF_PAD[6:1];      
    end     
    ...
    for (int i=0; i < 100; i++) begin
        ...
        #10000ns;
        rand_bit = `SOC_TOP.CLKSEL;
        if (`DIG_TOP.u_pinmux.o_OTP_UNLOCK !== rand_bit) begin
        `nnc_error("ATM0", $sformatf("`DIG_TOP.u_pinmux.o_OTP_UNLOCK:%b is not as expectation of CLKSEL: %b",`DIG_TOP.u_pinmux.o_OTP_UNLOCK, rand_bit))
        end
      release  `SOC_TOP.CLKSEL;              
    end 
    ...
    for (int i=0; i < 100; i++) begin
        ...
        `ifndef POSTLAYOUT_PG
           ...
           `nnc_error("ATM0", $sformatf("`DIG_TOP.u_pinmux.pin_rstn:%b is not as expectation of RESETn: %b",`DIG_TOP.u_pinmux.pin_rstn, rand_bit))
           end
           if (`DIG_TOP.u_pinmux.scan_rst_n !== rand_bit) begin
           `nnc_error("ATM0", $sformatf("`DIG_TOP.u_pinmux.scan_rst_n:%b is not as expectation of RESETn: %b",`DIG_TOP.u_pinmux.scan_rst_n, rand_bit))
           end
           if (`DIG_TOP.u_pinmux.otp_bist_resetn !== rand_bit) begin
           `nnc_error("ATM0", $sformatf("`DIG_TOP.u_pinmux.otp_bist_resetn:%b is not as expectation of RESETn: %b",`DIG_TOP.u_pinmux.otp_bist_resetn, rand_bit))
           end
        `else //in postlayout above all reset signals of pinmux has been optimized so consider only below one
           if (`DIG_TOP.u_pinmux.iopad_resetn_y !== rand_bit) begin
           `nnc_error("ATM0", $sformatf("`DIG_TOP.u_pinmux.iopad_resetn_y:%b is not as expectation of RESETn: %b",`DIG_TOP.u_pinmux.iopad_resetn_y, rand_bit))
...
`endif
    ...
    for (int i=0; i < 100; i++) begin
        ...
        `nnc_error("ATM0", $sformatf("`DIG_TOP.IOBUF_PAD[7] : %b is not as expectation of `DIG_TOP.u_pinmux.i_otp_vpp_en: %b",`SOC_TOP.IOBUF_PAD[7], rand_bit))
        end
        release `DIG_TOP.u_pinmux.i_otp_vpp_en;                      
    end  
    */
