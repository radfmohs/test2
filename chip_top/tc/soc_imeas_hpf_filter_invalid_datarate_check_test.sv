...
//--------------------------------------------------------------------------------------
// File Name	: soc_imeas_hpf_filter_invalid_datarate_check_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_imeas_hpf_filter_invalid_datarate_check_test                                             
// Designer	: shreeyal@nanochap.com                                                                 
// Date		: 10-11-2025                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_imeas_hpf_filter_invalid_datarate_check_test
`define TESTCFG soc_imeas_hpf_filter_invalid_datarate_check_test_cfg

class `TESTCFG extends soc_imeas_hpf_filter_base_test_cfg;
  ...
  function new (string name = "soc_imeas_hpf_filter_invalid_datarate_check_test_cfg");
    super.new(name);
  endfunction: new
  ...
  // -----------------------------------------------
  constraint c_imeas_sin_expected_freq { imeas_sin_expected_freq inside {[1000 : 2000]};} //sine freq * 1000
  ...
  constraint c_iclk_sel            { solve imeas_cic_rate before iclk_sel;
                                     ...
                                     (imeas_cic_rate == 8)  ->  iclk_sel inside {[8:11]};
                                     (imeas_cic_rate == 9)  ->  iclk_sel inside {[7:11]};
                                     (imeas_cic_rate == 10) ->  iclk_sel inside {[6:11]};
                                     (imeas_cic_rate == 11) ->  iclk_sel inside {[5:11]};
                                     (imeas_cic_rate == 12) ->  iclk_sel inside {[4:11]};
                                     (imeas_cic_rate == 13) ->  iclk_sel inside {[3:11]};}

  constraint c_imeas_en_dis_ch   {  imeas_en_dis_ch == 16'h0 ;} // all imeas eeg channel enabled 

  constraint c_hpf_filter_en_per_ch   {  hpf_filter_en_per_ch == 16'h0 ;} // all hpf filter channels enabled

  constraint c_sine_num_of_period   {  sine_num_of_period == 5; }  
...
class `TESTNAME extends soc_imeas_hpf_filter_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_imeas_hpf_filter_invalid_datarate_check_test start", UVM_LOW)
    ...
    fork
      begin
        //check hpf clk each intr, hpf clk should not be generated 
        @(`DUT_IF.hpf_clk);
	`nnc_error("TEST", $sformatf("ERROR:: HPF CLK enabled=%0b for invalid datarate = (%0f)Hz, iclk_div=%0d, cic_rate_sel=%0d", `DUT_IF.hpf_clk,`DUT_IF.imeas_samp_rate,`DUT_IF.iclk_sel,`DUT_IF.cic_rate))
      end
    join_none
    ...
    `nnc_info("SOC_TEST", "soc_imeas_hpf_filter_invalid_datarate_check_test end now", UVM_LOW)
    // ----------------------------------------------------------------------------------
    // End of adding test 
    // ----------------------------------------------------------------------------------
