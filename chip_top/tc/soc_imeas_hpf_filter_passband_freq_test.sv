...
//--------------------------------------------------------------------------------------
// File Name	: soc_imeas_hpf_filter_passband_freq_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_imeas_hpf_filter_passband_freq_test                                             
...
// NOTES:
// This test checks the High pass filter attenuation for Fc=0.5Hz -> which should be 3db

`define TESTNAME soc_imeas_hpf_filter_passband_freq_test
`define TESTCFG soc_imeas_hpf_filter_passband_freq_test_cfg
...
class `TESTCFG extends soc_imeas_hpf_filter_base_test_cfg;
  ...
  function new (string name = "soc_imeas_hpf_filter_passband_freq_test_cfg");
    super.new(name);
  endfunction: new
...
class `TESTNAME extends soc_imeas_hpf_filter_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_imeas_hpf_filter_passband_freq_test start", UVM_LOW)

    super.main_phase(phase);

    `nnc_info("SOC_TEST", "soc_imeas_hpf_filter_passband_freq_test end now", UVM_LOW)
    // ----------------------------------------------------------------------------------
    // End of adding test 
    // ----------------------------------------------------------------------------------
