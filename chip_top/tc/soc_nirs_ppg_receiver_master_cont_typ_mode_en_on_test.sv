         ...
         begin
           fork
              begin
                 ...
                 assert(top_test_cfg.randomize());                                        // if any of below thread hits chnage all channels A2D_* generation
                 //`DUT_IF.nirs_irefcoarse_length[7:0] = top_test_cfg.nirs_irefcoarse_length[7:0];
                 //`DUT_IF.nirs_irefcoarse_iref_delay[7:0] = top_test_cfg.nirs_irefcoarse_iref_delay[7:0];
                 //`DUT_IF.nirs_ireffine_length[7:0] = top_test_cfg.nirs_ireffine_length[7:0];
 
                 // before starting this iteration/thread
                 //foreach (curr_val[ch]) begin
               
                 //  do begin
                 //    assert(top_test_cfg.randomize());
                 //    top_test_cfg.fine_curr_va[ch] = top_test_cfg.fine_prev_val[ch];
                 //    top_test_cfg.coarse_curr_val[ch] === top_test_cfg.coarse_prev_val[ch];
                 //    top_test_cfg.length_prev_val[ch] === top_test_cfg.length_curr_val[ch]
                 //  end while ((fine_curr_valch] === top_test_cfg.fine_prev_val[ch]) ||  (top_test_cfg.coarse_curr_val[8] === top_test_cfg.coarse_prev_val[8]) || (top_test_cfg.length_prev_val[8] === top_test_cfg.length_curr_val[8]));
                 //                                                                                                                      
                 //    prev_val[ch] = curr_val[ch];
