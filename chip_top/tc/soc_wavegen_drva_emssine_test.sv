...
//--------------------------------------------------------------------------------------
// File Name	: soc_wavegen_drva_emssine_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_wavegen_drva_emssine_test                                             
...
//**********************************************************************************************
// NOTE : This test follows the same configuration sequence as sawtooth test, but additionally 
//        use wavegen interrupt feature to load new sawtooth wave data with different amplitude 
//        such that an ems output wave will be generated. Also this test enables Alternate mode
//        using config_reg.
//**********************************************************************************************
...
// -------------------------------------------------
`define TESTNAME soc_wavegen_drva_emssine_test
`define TESTCFG soc_wavegen_drva_emssine_test_cfg
...
class `TESTCFG extends soc_wavegen_base_test_cfg;
  ...
  function new (string name = "soc_wavegen_drva_emssine_test_cfg");
    super.new(name);
...
class `TESTNAME extends soc_wavegen_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_wavegen_drva_emssine_test start", NNC_LOW)
    ...
    // --------------------------------------------------------
    #10ms;
    `nnc_info("SOC_TEST", "soc_wavegen_drva_emssine_test end now", NNC_LOW)
  ...
  begin
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_ALT_LIM_REG01 + WG_BASE); no_of_bytes == 2; wr_data[0] == 8'h00; wr_data[1] == 8'h10;});
    `nnc_info("SOC_TEST", "Set no: of clocks for a period of alternating signal", NNC_LOW)//0x16 (16 clocks)
    `WR_BURST_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.wr_data);

    // --------------------------------------------------------
    // Write burst starting from ADDR_WG_DRV_ALT_SILENT_LIM_REG01
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_ALT_SILENT_LIM_REG01 + WG_BASE); no_of_bytes == 2; wr_data[0] == 8'h00; wr_data[1] == 8'h08;});
    `nnc_info("SOC_TEST", "Set no: of clocks to be silent after alternating", NNC_LOW)//0x0008 (8 clocks)
    `WR_BURST_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.wr_data);
...
`endif
    ...
    if(`DUT_IF.wg_drv_sel === 1 && d===0  && s===1)  @driver1_en_done;
	join
    wait(driver0_en_done);
	//for (int i=0; i<1; i++) begin //only one of the Driver A is working in this test case
	  ...
	  if ((top_test_cfg.rd_data[2][3]===1'b1) & (top_test_cfg.rd_data[2][2:0]===`DUT_IF.wg_drv_sel)) begin //also check if interrupt in this block is enabled (bit 8 is active)
		...
		if(top_test_cfg.rd_data[2][4]===1) begin
		    ...
		    if (top_test_cfg.rd_data[1] === 8'h20) begin //first address is at the second half of the wave; now load the first half
			// --------------------------------------------------------
    			// Write burst starting from ADDR_WG_DRV_INT_REG02 (set next address)
    			// --------------------------------------------------------
    			assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_INT_REG02 + (`WAVEGEN_1_ADDR_BASE * `DUT_IF.wg_drv_sel)); no_of_bytes == 2; wr_data[0] == 8'h1f; wr_data[1] == 8'h00;});
    			`nnc_info("SOC_TEST", "Configure interrupt register", NNC_LOW)//interrupt register set as first address 0; second address 31
    			`WR_BURST_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.wr_data);
			// --------------------------------------------------------
		    ...
		    else if (top_test_cfg.rd_data[1] === 8'h00) begin //first address is at the first half of the wave; now load the second half
			// --------------------------------------------------------
    			// Write burst starting from ADDR_WG_DRV_INT_REG02 (set next address)
    			// --------------------------------------------------------
    			assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_INT_REG02 + (`WAVEGEN_1_ADDR_BASE * `DUT_IF.wg_drv_sel)); no_of_bytes == 2; wr_data[0] == 8'h3f; wr_data[1] == 8'h20;});
    			`nnc_info("SOC_TEST", "Configure interrupt register", NNC_LOW)//interrupt register set as first address 32; second address 63
    			`WR_BURST_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.wr_data);
			// --------------------------------------------------------
    			// Write to ADDR_WG_DRV_INT_REG01 (clear the interrupt)
    			// --------------------------------------------------------
