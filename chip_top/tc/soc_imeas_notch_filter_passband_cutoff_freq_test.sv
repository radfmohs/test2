...
//--------------------------------------------------------------------------------------
// File Name	: soc_imeas_notch_filter_passband_cutoff_freq_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_imeas_notch_filter_passband_cutoff_freq_test                                             
// Designer	: shreeyal@nanochap.com                                                                 
// Date		: 10-11-2025                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_imeas_notch_filter_passband_cutoff_freq_test
`define TESTCFG soc_imeas_notch_filter_passband_cutoff_freq_test_cfg
...
class `TESTCFG extends soc_imeas_notch_filter_base_test_cfg;
  ...
  function new (string name = "soc_imeas_notch_filter_passband_cutoff_freq_test_cfg");
    super.new(name);
  endfunction: new
  ...
  // -----------------------------------------------
  //constraint c_imeas_sin_expected_freq {imeas_sin_expected_freq inside {[20: 46500]}
  //                                        || imeas_sin_expected_freq inside {[53750:256000]} ;} // Lower passband freq = 46.5 , upper passband freq = 53.75 //sine freq * 1000
  constraint c_imeas_sin_expected_freq { imeas_sin_expected_freq == 46500;} // Lower passband freq = 46.5 , upper passband freq = 53.75 //sine freq * 1000
...
class `TESTNAME extends soc_imeas_notch_filter_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_imeas_notch_filter_passband_cutoff_freq_test start", UVM_LOW)

    super.main_phase(phase);

    `nnc_info("SOC_TEST", "soc_imeas_notch_filter_passband_cutoff_freq_test end now", UVM_LOW)
    // ----------------------------------------------------------------------------------
    // End of adding test 
    // ----------------------------------------------------------------------------------
