...
class `TESTNAME extends soc_base_test;
  ...
  virtual task do_run;
	...
	top_test_cfg.GPIO_PU = {{16{1'b0}}, `SPI_TOP.spi_reg_u.gpio_pu_ctrl[2:0], {3{1'b0}}};  
...
`ifndef BEHAVIORAL
            temp_data = {`SOC_TOP.u_iopad_gpio_21_.PU,
                `SOC_TOP.u_iopad_gpio_20_.PU,
                `SOC_TOP.u_iopad_gpio_19_.PU,
...
`else
            temp_data = {`SOC_TOP.u_iopad_gpio[21].PU,
		`SOC_TOP.u_iopad_gpio[20].PU,
		`SOC_TOP.u_iopad_gpio[19].PU,
	    	`SOC_TOP.u_iopad_gpio[18].PU,
		`SOC_TOP.u_iopad_gpio[17].PU,
		`SOC_TOP.u_iopad_gpio[16].PU,
