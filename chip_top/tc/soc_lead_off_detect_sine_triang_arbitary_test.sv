...
//--------------------------------------------------------------------------------------
// File Name	: soc_lead_off_detect_sine_triang_arbitary_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_lead_off_detect_sine_triang_arbitary_test                                             
...
//************************************************************************************
// NOTE : This test checks the leadoff for continous sine/triangle/arbitary cases
//************************************************************************************

// =================================================
// Testcase name is defined
// -------------------------------------------------
`define TESTNAME soc_lead_off_detect_sine_triang_arbitary_test
`define TESTCFG soc_lead_off_detect_sine_triang_arbitary_test_cfg
...
class `TESTCFG extends soc_lead_off_detect_base_test_cfg;
  ...
  function new (string name = "soc_lead_off_detect_sine_triang_arbitary_test_cfg");
    super.new(name);
  endfunction: new
  ...
  //preload_sel
  constraint c_preload_sel     { preload_sel inside {[0:3]};} // 0: sine preloaded, 2: triangle preloaded, 3: user config sine

  constraint c_waveform_sel    { solve preload_sel before waveform_sel;
                                 preload_sel inside {0,2,3} -> waveform_sel inside {[0:2]};
                                 preload_sel == 1-> waveform_sel inside {[1:2]};} // only arbitary cases for pulse 
  // dac_sel
  constraint c_dac_sel     { dac_sel == 2'b11; }

  //points_sel
  constraint c_points_sel      { points_sel != 6;
...
class `TESTNAME extends soc_lead_off_detect_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_lead_off_detect_sine_triang_arbitary_test start", NNC_LOW)
    super.main_phase(phase);
    `nnc_info("SOC_TEST", "soc_lead_off_detect_sine_triang_arbitary_test end now", NNC_LOW)
