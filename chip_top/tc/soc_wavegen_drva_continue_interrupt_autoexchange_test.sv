...
//--------------------------------------------------------------------------------------
// File Name	: soc_wavegen_drva_continue_interrupt_autoexchange_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_wavegen_drva_continue_interrupt_autoexchange_test                                             
...
//**********************************************************************************************************
// NOTE : This test intention is to test the continue mode feature along with auto address exchange feature. 
//        The test intention is to check auto address swapping do not happen during 2nd address interrupt 
//        clear, but happens only during 1st address interrupt clear
//**********************************************************************************************************

// =================================================
// Testcase name is defined
// -------------------------------------------------
`define TESTNAME soc_wavegen_drva_continue_interrupt_autoexchange_test
`define TESTCFG soc_wavegen_drva_continue_interrupt_autoexchange_test_cfg
...
class `TESTCFG extends soc_wavegen_base_test_cfg;
  ...
  function new (string name = "soc_wavegen_drva_continue_interrupt_autoexchange_test_cfg");
    super.new(name);
...
class `TESTNAME extends soc_wavegen_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_wavegen_drva_continue_interrupt_autoexchange_test start", NNC_LOW)
    ...
    // --------------------------------------------------------
    #1ms;
    `nnc_info("SOC_TEST", "soc_wavegen_drva_continue_interrupt_autoexchange_test end now", NNC_LOW)
	  ...
	  if (drv_int_sts[`DUT_IF.wg_drv_sel] !== 0) begin
		...
		if(drv_int_sts[`DUT_IF.wg_drv_sel][0] === 1) begin
			...
			if(`DUT_IF.clear_intr_manual_or_auto === 0) begin//if manual with local register access
			  ...
			  fork
			  begin
    			  assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_INT_REG01 + (`WAVEGEN_1_ADDR_BASE * `DUT_IF.wg_drv_sel)); wr_data[0] == 8'h0f;});//auto_exchange bit[3]=1, clear both 1st & 2nd addr interrupt
