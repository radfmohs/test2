...
//--------------------------------------------------------------------------------------
// File Name	: soc_anac_short_leadoff_detection_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_anac_short_leadoff_detection_test                                             
...
//*******************************************************************************************
// NOTE : This test is used to check short detection as well as lead off detection using 
//        common configurations. Both circuits will be enabled to detect short & leadoff.
//*******************************************************************************************

// =================================================
// Testcase name is defined
// -------------------------------------------------
`define TESTNAME soc_anac_short_leadoff_detection_test
`define TESTCFG soc_anac_short_leadoff_detection_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  rand logic [15:0] alt_silent_lim;
       logic [31:0] CH1_timer_th;
       logic [31:0] CH2_timer_th;
       logic [31:0] short_CH1_counter_th;
       logic [31:0] short_CH2_counter_th;
       logic [31:0] leadoff_CH1_counter_th;
       logic [31:0] leadoff_CH2_counter_th;
  ...
  //rand logic        comp_reverse;
       ...
       logic [31:0] short_pulsewidth_ch1[3];//considering upto 3 waveform_sel
       logic [31:0] short_pulsewidth_ch2[3];//considering upto 3 waveform_sel
  ...
  rand logic [11:0] VDAC_DIN_CH2;
       bit          short_disable_wg_scb_drv_0;
       bit          short_disable_wg_scb_drv_1;
       bit          leadoff_disable_wg_scb_drv_0;
       bit          leadoff_disable_wg_scb_drv_1;
  // -----------------------------------------------
  // End of decalration of new variables 
  // ===============================================

  function new (string name = "soc_anac_short_leadoff_detection_test_cfg");
    super.new(name);
  ...
  //alt_en
  constraint c_alt_en          { alt_en == 1'b0;}//alternating mode disabled by default
  ...
  //alt_lim
  constraint c_alt_lim         { alt_lim inside {[2:32]};}
  ...
  //lead_off_ch0_int_en
  constraint c_lead_off_ch0_int_en { lead_off_ch0_int_en == 1'b1;}

  //lead_off_ch1_int_en
  constraint c_lead_off_ch1_int_en { lead_off_ch1_int_en == 1'b1;}
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task pre_reset_phase(nnc_phase phase);
    ...
    `DUT_IF.lead_off_stop_en_ch0 = top_test_cfg.stop_wavegen1;
    `DUT_IF.lead_off_stop_en_ch1 = top_test_cfg.stop_wavegen2;
  ...
  task generate_stimulus();
    ...
    fork
      begin
	    ...
	    `DUT_IF.register_val_ch1 = (normal_case === 1) ? 2'b10 : ((randomNumber0 <= 1) ?  2'b10 : $urandom_range(2'b00,2'b01));
            randomNumber0 = $urandom_range(10,0);
	    `DUT_IF.register_val_ch2 = (normal_case === 1) ? 2'b10 : ((randomNumber0 <= 1) ?  2'b10 : $urandom_range(2'b00,2'b01));
          end
      end
    join_none
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_anac_short_leadoff_detection_test start", NNC_LOW)

    // ==================================================================================
    // Please add your code of your test here
    // ----------------------------------------------------------------------------------

    if(counter_dbg_read_en === 1)
      normal_case = 1;
    ...
    //Estimate timer & counter TH settings
    threshold_window_estimation;

    //Configure short detection block
    short_config;

    //Configure leadoff detection block
    leadoff_config;
    ...
    fork
      short_wavegen_scb_dis;//disable wavegen SCB whenever short happens & wavegen stops
      short_wavegen_scb_en;//enable wavegen SCB whenever short happens & wavegen restarts
      leadoff_wavegen_scb_dis;//disable wavegen SCB whenever leadoff happens & wavegen stops
      leadoff_wavegen_scb_en;//enable wavegen SCB whenever leadoff happens & wavegen restarts
    ...
    `DUT_IF.A2D_comp_sel = top_test_cfg.A2D_comp_sel;//only used if analog model is not used

    if(counter_dbg_read_en === 1) begin
      normal_case = 1;
      //wait for both CH1&CH2 to complete short & leadoff time check window 
      fork
      begin
	wait(`DUT_IF.expected_anac_short_ch1_timer_th_cnt_flag === 1);
      end
      begin
	wait(`DUT_IF.expected_anac_short_ch2_timer_th_cnt_flag === 1);
      end
      begin
	wait(`DUT_IF.expected_leadoff_ch0_timer_th_cnt_flag === 1);
      end
      begin
	wait(`DUT_IF.expected_leadoff_ch1_timer_th_cnt_flag === 1);
      end
      join
      @(posedge`DUT_IF.sys_clk);//wait for 1 sysclk
      for(int i = 0; i < 4; i++) begin
      	`DUT_IF.short_leadoff_counter_cnt_debug_sel = i;
      	// ----------------------------------------------------------------------
      	// Write to SOC_COUNTER_CNT_DBG_SEL_REG
      	// ----------------------------------------------------------------------
      	assert(top_test_cfg.randomize() with {reg_addr == `SOC_COUNTER_CNT_DBG_SEL_REG; wr_data[0] == `DUT_IF.short_leadoff_counter_cnt_debug_sel;});
      	`WR_NORMAL_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);
      	//read the counter debug register to get the actual no: of responses for waveform during the time check window
      	// ------------------------------------------------------------------------------------------------------------------------------
      	// Read burst starting from SOC_COUNTER_CNT_DBG_0_REG
      	// ------------------------------------------------------------------------------------------------------------------------------
      	assert(top_test_cfg.randomize() with {reg_addr == `SOC_COUNTER_CNT_DBG_0_REG; no_of_bytes == 4;});
      	`RD_BURST_NORMAL_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.rd_data);
	if(i === 0) begin
	  top_test_cfg.short_CH1_counter_th[31:24] = top_test_cfg.rd_data[0];
	  top_test_cfg.short_CH1_counter_th[23:16] = top_test_cfg.rd_data[1];
	  top_test_cfg.short_CH1_counter_th[15:8] = top_test_cfg.rd_data[2];
	  top_test_cfg.short_CH1_counter_th[7:0] = top_test_cfg.rd_data[3];
	  `nnc_info("SOC_TEST", $sformatf("short CH1 counter dbg: %h",top_test_cfg.short_CH1_counter_th), NNC_LOW)
	end
	else if(i === 1) begin
	  top_test_cfg.short_CH2_counter_th[31:24] = top_test_cfg.rd_data[0];
	  top_test_cfg.short_CH2_counter_th[23:16] = top_test_cfg.rd_data[1];
	  top_test_cfg.short_CH2_counter_th[15:8] = top_test_cfg.rd_data[2];
	  top_test_cfg.short_CH2_counter_th[7:0] = top_test_cfg.rd_data[3];
	  `nnc_info("SOC_TEST", $sformatf("short CH2 counter dbg: %h",top_test_cfg.short_CH2_counter_th), NNC_LOW)
	end
	else if(i === 2) begin
	  top_test_cfg.leadoff_CH1_counter_th[31:24] = top_test_cfg.rd_data[0];
	  top_test_cfg.leadoff_CH1_counter_th[23:16] = top_test_cfg.rd_data[1];
	  top_test_cfg.leadoff_CH1_counter_th[15:8] = top_test_cfg.rd_data[2];
	  top_test_cfg.leadoff_CH1_counter_th[7:0] = top_test_cfg.rd_data[3];
	  `nnc_info("SOC_TEST", $sformatf("leadoff CH1 counter dbg: %h",top_test_cfg.leadoff_CH1_counter_th), NNC_LOW)
	end
	else begin
	  top_test_cfg.leadoff_CH2_counter_th[31:24] = top_test_cfg.rd_data[0];
	  top_test_cfg.leadoff_CH2_counter_th[23:16] = top_test_cfg.rd_data[1];
	  top_test_cfg.leadoff_CH2_counter_th[15:8] = top_test_cfg.rd_data[2];
	  top_test_cfg.leadoff_CH2_counter_th[7:0] = top_test_cfg.rd_data[3];
	  `nnc_info("SOC_TEST", $sformatf("leadoff CH2 counter dbg: %h",top_test_cfg.leadoff_CH2_counter_th), NNC_LOW)
	end
      end

      // -----------------------------------------------------------------------
      // Write to SOC_WAVEGEN_GLOBAL_REG
      // -----------------------------------------------------------------------
      assert(top_test_cfg.randomize() with {reg_addr == `SOC_WAVEGEN_GLOBAL_REG; wr_data[0] == 8'h00;});
      `nnc_info("SOC_TEST", "Disable wavegen drivers", NNC_LOW)
      `WAVEGEN_SCB_DRV_0_EN = 1'b0;//Disable
      `WAVEGEN_SCB_DRV_1_EN = 1'b0;//Disable
      `WR_NORMAL_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);

      //considering 1 extra clk delay after restart wavegen
      if(`DUT_IF.preload_sel === 2'b01) begin
      //pulse case
      	`DUT_IF.anac_short_CH1_counter_TH = top_test_cfg.short_CH1_counter_th - 4;
      	`DUT_IF.anac_short_CH2_counter_TH = top_test_cfg.short_CH2_counter_th - 4;
      	`DUT_IF.lead_off_counter_th_dac0 = top_test_cfg.leadoff_CH1_counter_th - 4;
      	`DUT_IF.lead_off_counter_th_dac1 = top_test_cfg.leadoff_CH2_counter_th - 4;
      end
      else begin
      //sine/tri/arb case
	`DUT_IF.anac_short_CH1_counter_TH = top_test_cfg.short_CH1_counter_th - 3;
      	`DUT_IF.anac_short_CH2_counter_TH = top_test_cfg.short_CH2_counter_th - 3;
      	`DUT_IF.lead_off_counter_th_dac0 = top_test_cfg.leadoff_CH1_counter_th - 3;
      	`DUT_IF.lead_off_counter_th_dac1 = top_test_cfg.leadoff_CH2_counter_th - 3;
      end

      `nnc_info("SOC_TEST", $sformatf("Setup short CH1 Counter Threhold: %d", `DUT_IF.anac_short_CH1_counter_TH), NNC_LOW)
      // -----------------------------------------------------------------------
      // Write burst starting from SOC_ANA_STIM_CH1_COUNTER_CNT_TH00_REG for CH1
      // -----------------------------------------------------------------------
      assert(top_test_cfg.randomize() with {reg_addr == `SOC_ANA_STIM_CH1_COUNTER_CNT_TH00_REG; no_of_bytes == 4;  wr_data[0] == `DUT_IF.anac_short_CH1_counter_TH[31:24]; wr_data[1] == `DUT_IF.anac_short_CH1_counter_TH[23:16]; wr_data[2] == `DUT_IF.anac_short_CH1_counter_TH[15:8]; wr_data[3] == `DUT_IF.anac_short_CH1_counter_TH[7:0];});
     `WR_BURST_NORMAL_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.wr_data);

      `nnc_info("SOC_TEST", $sformatf("Setup short CH2 Counter Threhold: %d", `DUT_IF.anac_short_CH2_counter_TH), NNC_LOW)
      // -----------------------------------------------------------------------
      // Write burst starting from SOC_ANA_STIM_CH2_COUNTER_CNT_TH00_REG for CH2
      // -----------------------------------------------------------------------
      assert(top_test_cfg.randomize() with {reg_addr == `SOC_ANA_STIM_CH2_COUNTER_CNT_TH00_REG; no_of_bytes == 4;  wr_data[0] == `DUT_IF.anac_short_CH2_counter_TH[31:24]; wr_data[1] == `DUT_IF.anac_short_CH2_counter_TH[23:16]; wr_data[2] == `DUT_IF.anac_short_CH2_counter_TH[15:8]; wr_data[3] == `DUT_IF.anac_short_CH2_counter_TH[7:0];});
      `WR_BURST_NORMAL_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.wr_data);

      `nnc_info("SOC_TEST", $sformatf("Setup leadoff CH1 Counter Threhold: %d", `DUT_IF.lead_off_counter_th_dac0), NNC_LOW)
      // -----------------------------------------------------------------------------
      ...
      `nnc_info("SOC_TEST", $sformatf("Setup leadoff CH2 Counter Threhold: %d", `DUT_IF.lead_off_counter_th_dac1), NNC_LOW)
      // -----------------------------------------------------------------------------
      ...
      normal_case = 0;
      //wait for 2 sysclk before restaring wavegen
      ...
      `nnc_info("SOC_TEST", "Enable wavegen drivers", NNC_LOW)
	...
	wait(`SOC_TB.INTB === 0);
      ...
      //Only automatic clearing considered; reading sts bit supposed to clear interrupt
	...
	if(top_test_cfg.rd_data[0][5] === 1'b1) begin
	  `nnc_info("SOC_TEST", "SHORT CH1 int sts is set!", NNC_LOW)
	  wait(`ANAC_TOP.spi_anac.ana_comp_ch_intr_sts[0] === 0);
	  `nnc_info("SOC_TEST", "SHORT CH1 int cleared!", NNC_LOW)
	end
	if(top_test_cfg.rd_data[0][6] === 1'b1) begin
	  `nnc_info("SOC_TEST", "SHORT CH2 int sts is set!", NNC_LOW)
	  wait(`ANAC_TOP.spi_anac.ana_comp_ch_intr_sts[1] === 0);
	  `nnc_info("SOC_TEST", "SHORT CH2 int cleared!", NNC_LOW)
	end
	if(top_test_cfg.rd_data[0][3] === 1'b1) begin
	  `nnc_info("SOC_TEST", "LEAD_OFF CH1 int sts is set!", NNC_LOW)
	  wait(`LEADOFF_TOP_0.lead_off_result === 0);
	  `nnc_info("SOC_TEST", "LEAD_OFF CH1 int cleared!", NNC_LOW)
	end
	if(top_test_cfg.rd_data[0][4] === 1'b1) begin
	  `nnc_info("SOC_TEST", "LEAD_OFF CH2 int sts is set!", NNC_LOW)
	  wait(`LEADOFF_TOP_1.lead_off_result === 0);
	  `nnc_info("SOC_TEST", "LEAD_OFF CH2 int cleared!", NNC_LOW)
	end
      end
    ...
    // --------------------------------------------------------
    #10000ns;
    `nnc_info("SOC_TEST", "soc_anac_short_leadoff_detection_test end now", NNC_LOW)
  ...
  begin
    ...
    `nnc_info("SOC_TEST", $sformatf("half_period_limit: %d, top_test_cfg.clk_freq: %d,`DUT_IF.pclk_sel=%0d ", top_test_cfg.half_period_limit, top_test_cfg.clk_freq,`DUT_IF.pclk_sel), NNC_LOW)
  ...
  begin
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_CONFIG_REG0 + WG_BASE); wr_data[0] == {`DUT_IF.pos_ena, 1'b1, 1'b0, `DUT_IF.alt_en, 1'b1, `DUT_IF.silent_en, `DUT_IF.neg_ena, `DUT_IF.rest_en};});
    ...
    `nnc_info("SOC_TEST", "Set no: of clocks for a period of alternating signal", NNC_LOW)
    `WR_BURST_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.wr_data);
    ...
    `nnc_info("SOC_TEST", "Set no: of clocks to be silent after alternating", NNC_LOW)
    `WR_BURST_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.wr_data);
  ...
  task short_wavegen_scb_dis;
  begin
    forever @(posedge `ANAC_TOP.spi_anac.ana_comp_ch_intr_sts[0] or posedge `ANAC_TOP.spi_anac.ana_comp_ch_intr_sts[1]) begin
      if((`DUT_IF.stop_wave1 === 1) && (`ANAC_TOP.spi_anac.ana_comp_ch_intr_sts[0] === 1) && (top_test_cfg.short_disable_wg_scb_drv_0 === 0)) begin
	`WAVEGEN_SCB_DRV_0_EN = 1'b0;//Disable
        top_test_cfg.short_disable_wg_scb_drv_0 = 1;
    	`nnc_info("SOC_TEST", "WAVGEN_SCB Disabled for drv 0 !", NNC_LOW)
      end
      if((`DUT_IF.stop_wave2 === 1) && (`ANAC_TOP.spi_anac.ana_comp_ch_intr_sts[1] === 1) && (top_test_cfg.short_disable_wg_scb_drv_1 === 0)) begin
	`WAVEGEN_SCB_DRV_1_EN = 1'b0;//Disable
        top_test_cfg.short_disable_wg_scb_drv_1 = 1;
  ...
  task short_wavegen_scb_en;
  begin
    forever @(negedge `ANAC_TOP.spi_anac.ana_comp_ch_intr_sts[0] or negedge `ANAC_TOP.spi_anac.ana_comp_ch_intr_sts[1]) begin
      if((`DUT_IF.stop_wave1 === 1) && (`ANAC_TOP.spi_anac.ana_comp_ch_intr_sts[0] === 0) && (top_test_cfg.short_disable_wg_scb_drv_0 === 1)) begin
	`WAVEGEN_SCB_DRV_0_EN = 1'b1;//Enable
        top_test_cfg.short_disable_wg_scb_drv_0 = 0;
    	`nnc_info("SOC_TEST", "WAVGEN_SCB Enabled for drv 0 !", NNC_LOW)
      end
      if((`DUT_IF.stop_wave2 === 1) && (`ANAC_TOP.spi_anac.ana_comp_ch_intr_sts[1] === 0) && (top_test_cfg.short_disable_wg_scb_drv_1 === 1)) begin
	`WAVEGEN_SCB_DRV_1_EN = 1'b1;//Enable
        top_test_cfg.short_disable_wg_scb_drv_1 = 0;
  ...
  task leadoff_wavegen_scb_dis;
  begin
    forever @(posedge `LEADOFF_TOP_0.lead_off_result or posedge `LEADOFF_TOP_1.lead_off_result) begin
      if((`DUT_IF.lead_off_stop_en_ch0 === 1) && (`LEADOFF_TOP_0.lead_off_result === 1) && (top_test_cfg.leadoff_disable_wg_scb_drv_0 === 0)) begin
	`WAVEGEN_SCB_DRV_0_EN = 1'b0;//Disable
        top_test_cfg.leadoff_disable_wg_scb_drv_0 = 1;
    	`nnc_info("SOC_TEST", "WAVGEN_SCB Disabled for drv 0 !", NNC_LOW)
      end
      if((`DUT_IF.lead_off_stop_en_ch1 === 1) && (`LEADOFF_TOP_1.lead_off_result === 1) && (top_test_cfg.leadoff_disable_wg_scb_drv_1 === 0)) begin
	`WAVEGEN_SCB_DRV_1_EN = 1'b0;//Disable
        top_test_cfg.leadoff_disable_wg_scb_drv_1 = 1;
    	`nnc_info("SOC_TEST", "WAVGEN_SCB Disabled for drv 1 !", NNC_LOW)
      end
    end
  end
  ...
  task leadoff_wavegen_scb_en;
  begin
    forever @(negedge `LEADOFF_TOP_0.lead_off_result or negedge `LEADOFF_TOP_1.lead_off_result) begin
      if((`DUT_IF.lead_off_stop_en_ch0 === 1) && (`LEADOFF_TOP_0.lead_off_result === 0) && (top_test_cfg.leadoff_disable_wg_scb_drv_0 === 1)) begin
	`WAVEGEN_SCB_DRV_0_EN = 1'b1;//Enable
        top_test_cfg.leadoff_disable_wg_scb_drv_0 = 0;
    	`nnc_info("SOC_TEST", "WAVGEN_SCB Enabled for drv 0 !", NNC_LOW)
      end
      if((`DUT_IF.lead_off_stop_en_ch1 === 1) && (`LEADOFF_TOP_1.lead_off_result === 0) && (top_test_cfg.leadoff_disable_wg_scb_drv_1 === 1)) begin
	`WAVEGEN_SCB_DRV_1_EN = 1'b1;//Enable
        top_test_cfg.leadoff_disable_wg_scb_drv_1 = 0;
    	`nnc_info("SOC_TEST", "WAVGEN_SCB Enabled for drv 1 !", NNC_LOW)
      end
    end
  end
  ...
  begin
    forever @(posedge `SOC_TB.INTB or negedge `SOC_TB.INTB) begin
      if((`DUT_IF.intr_length_slct_level_or_pulse === 0) && (`DUT_IF.int_active_level_high_or_low === 1)) begin//if active high level INTB is selected
        if(`SOC_TB.INTB !== (`ANAC_TOP.genblk1[0].u_anac_short_dtct_ch.o_ana_stimu_chx_intr_pin | `ANAC_TOP.genblk1[1].u_anac_short_dtct_ch.o_ana_stimu_chx_intr_pin | `LEADOFF_TOP_0.lead_off_result | `LEADOFF_TOP_1.lead_off_result))
  ...
  task threshold_window_estimation;
  begin
    /********************************************************* Calculate the timer_TH to be set for CH1 & CH2 *********************************************/
    if(`DUT_IF.alt_en === 1'b0) begin
      if((`DUT_IF.short_detect_by_lead_off_en === 1) || (`DUT_IF.lead_off_detect_by_short_circuit_en === 0)) begin//for SHORT detection
       if(`DUT_IF.D2A_comp_stim0_1_sel === 0) begin
	top_test_cfg.short_pulsewidth_ch1[0] = top_test_cfg.hlf_wave0_lim[0] * `DUT_IF.point_cfg_val;
	top_test_cfg.short_pulsewidth_ch1[1] = top_test_cfg.hlf_wave1_lim[0] * `DUT_IF.point_cfg_val;
	top_test_cfg.short_pulsewidth_ch1[2] = top_test_cfg.hlf_wave2_lim[0] * `DUT_IF.point_cfg_val;
       end
       else begin
	top_test_cfg.short_pulsewidth_ch1[0] = top_test_cfg.neg_hlf_wave0_lim[0] * `DUT_IF.point_cfg_val;
	top_test_cfg.short_pulsewidth_ch1[1] = top_test_cfg.neg_hlf_wave1_lim[0] * `DUT_IF.point_cfg_val;
	top_test_cfg.short_pulsewidth_ch1[2] = top_test_cfg.neg_hlf_wave2_lim[0] * `DUT_IF.point_cfg_val;
       end
       if(`DUT_IF.D2A_comp_stim2_3_sel === 0) begin
	top_test_cfg.short_pulsewidth_ch2[0] = top_test_cfg.hlf_wave0_lim[1] * `DUT_IF.point_cfg_val;
	top_test_cfg.short_pulsewidth_ch2[1] = top_test_cfg.hlf_wave1_lim[1] * `DUT_IF.point_cfg_val;
	top_test_cfg.short_pulsewidth_ch2[2] = top_test_cfg.hlf_wave2_lim[1] * `DUT_IF.point_cfg_val;
       end
       else begin
	top_test_cfg.short_pulsewidth_ch2[0] = top_test_cfg.neg_hlf_wave0_lim[1] * `DUT_IF.point_cfg_val;
	top_test_cfg.short_pulsewidth_ch2[1] = top_test_cfg.neg_hlf_wave1_lim[1] * `DUT_IF.point_cfg_val;
	top_test_cfg.short_pulsewidth_ch2[2] = top_test_cfg.neg_hlf_wave2_lim[1] * `DUT_IF.point_cfg_val;
       end
      end
      if((`DUT_IF.short_detect_by_lead_off_en === 0) || (`DUT_IF.lead_off_detect_by_short_circuit_en === 1)) begin//for LEADOFF detection
    ...
    else begin//alternating mode alternates during positive side(state:S1) only
	top_test_cfg.short_pulsewidth_ch1[0] = top_test_cfg.hlf_wave0_lim[0] * `DUT_IF.point_cfg_val;
	top_test_cfg.short_pulsewidth_ch1[1] = top_test_cfg.hlf_wave1_lim[0] * `DUT_IF.point_cfg_val;
	top_test_cfg.short_pulsewidth_ch1[2] = top_test_cfg.hlf_wave2_lim[0] * `DUT_IF.point_cfg_val;
	top_test_cfg.short_pulsewidth_ch2[0] = top_test_cfg.hlf_wave0_lim[1] * `DUT_IF.point_cfg_val;
	...
	//pulsewidth
	top_test_cfg.short_pulse_width_ch1 = top_test_cfg.short_pulsewidth_ch1[0];
	top_test_cfg.short_pulse_width_ch2 = top_test_cfg.short_pulsewidth_ch2[0];
	...
	//pulsewidth
	top_test_cfg.short_pulse_width_ch1 = top_test_cfg.short_pulsewidth_ch1[0] + top_test_cfg.short_pulsewidth_ch1[1];
	top_test_cfg.short_pulse_width_ch2 = top_test_cfg.short_pulsewidth_ch2[0] + top_test_cfg.short_pulsewidth_ch2[1];
	...
	//pulsewidth
    	top_test_cfg.short_pulse_width_ch1 = top_test_cfg.short_pulsewidth_ch1[0] + top_test_cfg.short_pulsewidth_ch1[1] + top_test_cfg.short_pulsewidth_ch1[2];
	top_test_cfg.short_pulse_width_ch2 = top_test_cfg.short_pulsewidth_ch2[0] + top_test_cfg.short_pulsewidth_ch2[1] + top_test_cfg.short_pulsewidth_ch2[2];
    ...
    top_test_cfg.CH1_timer_th = (`DUT_IF.no_of_cycles_CH1 + `DUT_IF.no_of_waveforms)/(`DUT_IF.no_of_waveforms+1) * top_test_cfg.period_ch1;
    top_test_cfg.CH2_timer_th = (`DUT_IF.no_of_cycles_CH2 + `DUT_IF.no_of_waveforms)/(`DUT_IF.no_of_waveforms+1) * top_test_cfg.period_ch2;
    `DUT_IF.anac_short_CH1_timer_TH = top_test_cfg.CH1_timer_th + `DUT_IF.DELAY_lim[0];
    `DUT_IF.anac_short_CH2_timer_TH = top_test_cfg.CH2_timer_th + `DUT_IF.DELAY_lim[0];
    `DUT_IF.lead_off_timer_cnt_dac0 = top_test_cfg.CH1_timer_th + `DUT_IF.DELAY_lim[0];
    `DUT_IF.lead_off_timer_cnt_dac1 = top_test_cfg.CH2_timer_th + `DUT_IF.DELAY_lim[0];

    /********************************************************* Calculate the counter_TH to be set for CH1 & CH2 *********************************************/
    //considering 1 extra clk delay when pclk_div is 0
    if(`DUT_IF.preload_sel === 2'b01) begin
    //pulse case
    	top_test_cfg.short_CH1_counter_th = ((`DUT_IF.no_of_cycles_CH1 + `DUT_IF.no_of_waveforms)/(`DUT_IF.no_of_waveforms+1) * top_test_cfg.short_pulse_width_ch1) - 4;
    	top_test_cfg.short_CH2_counter_th = ((`DUT_IF.no_of_cycles_CH2 + `DUT_IF.no_of_waveforms)/(`DUT_IF.no_of_waveforms+1) * top_test_cfg.short_pulse_width_ch2) - 4;
    	top_test_cfg.leadoff_CH1_counter_th = ((`DUT_IF.no_of_cycles_CH1 + `DUT_IF.no_of_waveforms)/(`DUT_IF.no_of_waveforms+1) * top_test_cfg.leadoff_pulse_width_ch1) - 4;
    	top_test_cfg.leadoff_CH2_counter_th = ((`DUT_IF.no_of_cycles_CH2 + `DUT_IF.no_of_waveforms)/(`DUT_IF.no_of_waveforms+1) * top_test_cfg.leadoff_pulse_width_ch2) - 4;
    end
    else begin
    //sine/tri/arb case
    	top_test_cfg.short_CH1_counter_th = (((`DUT_IF.no_of_cycles_CH1 + `DUT_IF.no_of_waveforms)/(`DUT_IF.no_of_waveforms+1) * top_test_cfg.short_pulse_width_ch1)/2) - 2;
    	top_test_cfg.short_CH2_counter_th = (((`DUT_IF.no_of_cycles_CH2 + `DUT_IF.no_of_waveforms)/(`DUT_IF.no_of_waveforms+1) * top_test_cfg.short_pulse_width_ch2)/2) - 2;
    	top_test_cfg.leadoff_CH1_counter_th = (((`DUT_IF.no_of_cycles_CH1 + `DUT_IF.no_of_waveforms)/(`DUT_IF.no_of_waveforms+1) * top_test_cfg.leadoff_pulse_width_ch1)/2) - 2;
    	top_test_cfg.leadoff_CH2_counter_th = (((`DUT_IF.no_of_cycles_CH2 + `DUT_IF.no_of_waveforms)/(`DUT_IF.no_of_waveforms+1) * top_test_cfg.leadoff_pulse_width_ch2)/2) - 2;
    end
    ...
    `DUT_IF.lead_off_counter_th_dac0 = top_test_cfg.leadoff_CH1_counter_th;
    `DUT_IF.lead_off_counter_th_dac1 = top_test_cfg.leadoff_CH2_counter_th;
  end
  endtask
  ...
  begin
    ...
    // ---------------------------------------------------------------------
    // Write burst starting from SOC_ANA_STIM_CH1_TIMER_CNT_TH00_REG for CH1
    // ---------------------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == `SOC_ANA_STIM_CH1_TIMER_CNT_TH00_REG; no_of_bytes == 4;  wr_data[0] == `DUT_IF.anac_short_CH1_timer_TH[31:24]; wr_data[1] == `DUT_IF.anac_short_CH1_timer_TH[23:16]; wr_data[2] == `DUT_IF.anac_short_CH1_timer_TH[15:8]; wr_data[3] == `DUT_IF.anac_short_CH1_timer_TH[7:0];});
    `WR_BURST_NORMAL_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.wr_data);

    `nnc_info("SOC_TEST", $sformatf("Setup CH2 Timer Threhold: %d", `DUT_IF.anac_short_CH2_timer_TH), NNC_LOW)
    // ---------------------------------------------------------------------
    // Write burst starting from SOC_ANA_STIM_CH2_TIMER_CNT_TH00_REG for CH2
    // ---------------------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == `SOC_ANA_STIM_CH2_TIMER_CNT_TH00_REG; no_of_bytes == 4;  wr_data[0] == `DUT_IF.anac_short_CH2_timer_TH[31:24]; wr_data[1] == `DUT_IF.anac_short_CH2_timer_TH[23:16]; wr_data[2] == `DUT_IF.anac_short_CH2_timer_TH[15:8]; wr_data[3] == `DUT_IF.anac_short_CH2_timer_TH[7:0];});
    `WR_BURST_NORMAL_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.wr_data);
  ...
  task leadoff_config;
    /*************************************************************************************************************************************************************************************************************/
    /********************************************************************************** Configurations for LEADOFF DETECTION *************************************************************************************/
    /*************************************************************************************************************************************************************************************************************/
    // --------------------------------------------------------
    ...
    `WR_NORMAL_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);
    if(`DUT_IF.short_detect_by_lead_off_en === 0)
	   `nnc_info("SOC_TEST", "LEAD_OFF DETECTION BLOCK PERFORMS LEAD_OFF!", NNC_LOW)
    else
	   `nnc_info("SOC_TEST", "LEAD_OFF DETECTION BLOCK PERFORMS SHORT!", NNC_LOW)
    ...
    `nnc_info("SOC_TEST", $sformatf("Setup CH0 Counter Threhold: %d", `DUT_IF.lead_off_counter_th_dac0), NNC_LOW)
