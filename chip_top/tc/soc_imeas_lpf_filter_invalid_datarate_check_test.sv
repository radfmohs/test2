...
//--------------------------------------------------------------------------------------
// File Name	: soc_imeas_lpf_filter_invalid_datarate_check_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_imeas_lpf_filter_invalid_datarate_check_test                                             
// Designer	: shreeyal@nanochap.com                                                                 
// Date		: 10-11-2025                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_imeas_lpf_filter_invalid_datarate_check_test
`define TESTCFG soc_imeas_lpf_filter_invalid_datarate_check_test_cfg
...
class `TESTCFG extends soc_imeas_lpf_filter_base_test_cfg;
  ...
  function new (string name = "soc_imeas_lpf_filter_invalid_datarate_check_test_cfg");
    super.new(name);
  endfunction: new
  ...
  constraint c_lpf_filter_en_per_ch   {  lpf_filter_en_per_ch == 16'h0 ;} // all lpf filter channels enabled
...
class `TESTNAME extends soc_imeas_lpf_filter_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_imeas_lpf_filter_invalid_datarate_check_test start", UVM_LOW)
    ...
    fork
      begin
        //check lpf clk each intr, lpf clk should not be generated 
        @(`DUT_IF.lpf_clk);
	`nnc_error("TEST", $sformatf("ERROR:: LPF CLK enabled=%0b for invalid datarate = (%0f)Hz, iclk_div=%0d, cic_rate_sel=%0d", `DUT_IF.lpf_clk,`DUT_IF.imeas_samp_rate,`DUT_IF.iclk_sel,`DUT_IF.cic_rate))
      end
    join_none
    ...
    `nnc_info("SOC_TEST", "soc_imeas_lpf_filter_invalid_datarate_check_test end now", UVM_LOW)
    // ----------------------------------------------------------------------------------
    // End of adding test 
    // ----------------------------------------------------------------------------------
