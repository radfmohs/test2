...
// Description	: Testcase soc_pinmux_debugmode_atm14_test                                             
// Designer	: ddang@nanochap.com                                                                 
// Date		: 29-11-1623                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_pinmux_debugmode_atm14_test
`define TESTCFG soc_pinmux_debugmode_atm14_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_pinmux_debugmode_atm14_test_cfg");
    super.new(name);
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_pinmux_debugmode_atm14_test start", NNC_LOW)
...
`endif  
    ...
    if (top_test_cfg.atm !== (30'b10_0000_0000_0000_0000_0000_0000_0000 >> `SOC_TOP.IOBUF_PAD[14:10]))
      begin
        `nnc_error("ATM14", $sformatf("ATM[0:28] = %b is not as expectation of ATM = %b", top_test_cfg.atm, (30'b10_0000_0000_0000_0000_0000_0000_0000 >> `SOC_TOP.IOBUF_PAD[14:10])))
      end  

    //Check default signal  
    if (`ANA_TOP.D2A_BIST_SEL !== 5'b01110)
      begin
        `nnc_error("ATM14", $sformatf("D2A_BIST_SEL = %b is not as expectation of 5'b01110", `ANA_TOP.D2A_BIST_SEL))
      end
    if (`ANA_TOP.D2A_RLD_ELECTRODE_EN !== 1'b1)
      begin
        `nnc_error("ATM14", $sformatf("D2A_RLD_ELECTRODE_EN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_RLD_ELECTRODE_EN))
      end
    if (`ANA_TOP.D2A_VCMGENBUFF_EN !== 1'b1)
      begin
        `nnc_error("ATM14", $sformatf("D2A_VCMGENBUFF_EN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_VCMGENBUFF_EN))
      end
    if (`ANA_TOP.D2A_SDMVCMBUFF_EN !== 1'b1)
      begin
        `nnc_error("ATM14", $sformatf("D2A_SDMVCMBUFF_EN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_SDMVCMBUFF_EN))
      end
    ...
    if (`ANA_TOP.D2A_EEGLNA_EN[8] !== 1'b1)
      begin
        `nnc_error("ATM14", $sformatf("D2A_EEGLNA_EN[8] = %b is not as expectation of 1'b1", `ANA_TOP.D2A_EEGLNA_EN[8]))
      end
    if (`ANA_TOP.D2A_EEGPGA_EN[8] !== 1'b1)
      begin
        `nnc_error("ATM13", $sformatf("D2A_EEGPGA_EN[8] = %b is not as expectation of 1'b1", `ANA_TOP.D2A_EEGPGA_EN[8]))
      end
    if (`ANA_TOP.D2A_SDMBUFF_EN[8] !== 1'b1)
      begin
        `nnc_error("ATM14", $sformatf("D2A_SDMBUFF_EN[8] = %b is not as expectation of 1'b1", `ANA_TOP.D2A_SDMBUFF_EN[8]))
      end
    if (`ANA_TOP.D2A_SDMEN[8] !== 1'b1)
      begin
        `nnc_error("ATM14", $sformatf("D2A_SDMEN[8] = %b is not as expectation of 1'b1", `ANA_TOP.D2A_SDMEN[8]))
      end
    if (`ANA_TOP.D2A_BIST_EN !== 1'b1)
      begin
        `nnc_error("ATM14", $sformatf("D2A_BIST_EN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_BIST_EN))
      end  
    if (`ANA_TOP.D2A_OSC8MHZEN !== 1'b1)
      begin
        `nnc_error("ATM14", $sformatf("D2A_OSC8MHZEN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_OSC8MHZEN))
      end 
    if(`ANA_TOP.D2A_BGBUFFER_CPTEST_EN != 1'b1)
      begin
        `nnc_error("ATM14", $sformatf("D2A_BGBUFFER_CPTEST_EN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_BGBUFFER_CPTEST_EN))
      end
...
`ifdef BEHAVIORAL             
        if (`ANA_WRAPPER_TOP.pinmux_if.D2A_TRIM_SIG[14][7:0]!== {rand_num[7:0]}) begin
        `nnc_error("ATM14", $sformatf("`SOC_TOP.pinmux_if.D2A_TRIM_SIG[14][7:0] = %b is not as expectation of = %b", `ANA_WRAPPER_TOP.pinmux_if.D2A_TRIM_SIG[14][7:0], {rand_num[7:0]}));
        end
`else
        if (`ANA_WRAPPER_TOP.pinmux_if_D2A_TRIM_SIG[7:0]!== {rand_num[7:0]}) begin
        `nnc_error("ATM14", $sformatf("`SOC_TOP.pinmux_if.D2A_TRIM_SIG[7:0] = %b is not as expectation of = %b", `ANA_WRAPPER_TOP.pinmux_if_D2A_TRIM_SIG[7:0], {rand_num[7:0]}));
        end
`endif
        if (`ANA_TOP.D2A_TRIM7_SIG_SPARE!== {rand_num[7:0]}) begin
        `nnc_error("ATM14", $sformatf("`ANA_TOP.D2A_TRIM7_SIG_SPARE = %b is not as expectation of = %b", `ANA_TOP.D2A_TRIM7_SIG_SPARE, {rand_num[7:0]}));
        end
    ...
    for (int i=0; i < 100; i++) begin
        ...
        `nnc_error("ATM14", $sformatf("`EPROM_TOP.unlock_gpio:%b is not as expectation of CLKSEL: %b",`EPROM_TOP.unlock_gpio, rand_bit))
        end
    ...
    for (int i=0; i < 100; i++) begin
        ...
        `ifndef POSTLAYOUT_PG
           ...
           `nnc_error("ATM14", $sformatf("`RST_CTRL_TOP.ext_resetn:%b is not as expectation of RESETn: %b",`RST_CTRL_TOP.ext_resetn, rand_bit))
           end
           if (`RST_CTRL_TOP.scan_rst_n !== rand_bit) begin
           `nnc_error("ATM14", $sformatf("`RST_CTRL_TOP.scan_rst_n:%b is not as expectation of RESETn: %b",`RST_CTRL_TOP.scan_rst_n, rand_bit))
           end
           if (`RST_CTRL_TOP.otp_bist_resetn !== rand_bit) begin
           `nnc_error("ATM14", $sformatf("`RST_CTRL_TOP.otp_bist_resetn:%b is not as expectation of RESETn: %b",`RST_CTRL_TOP.otp_bist_resetn, rand_bit))
           end
        `else //in postlayout above all reset signals of pinmux has been optimized so consider only below one
           ...
           `nnc_error("ATM14", $sformatf("`DIG_TOP.u_pinmux.iopad_resetn_y:%b is not as expectation of RESETn: %b",`DIG_TOP.u_pinmux.iopad_resetn_y, rand_bit))
           end
           if (`RST_CTRL_TOP.otp_bist_resetn!== rand_bit) begin
             `nnc_error("ATM14", $sformatf("`DIG_TOP.u_pinmux.otp_bist_resetn = %b is not as expectation of = %b", `RST_CTRL_TOP.otp_bist_resetn, rand_bit));
           end
           if (`DIG_TOP.scan_rst_n !== rand_bit) begin
            `nnc_error("ATM14", $sformatf("`DIG_TOP.u_pinmux.scan_rst_n = %b is not as expectation of scan_rst_n = %b",`DIG_TOP.scan_rst_n, `SOC_TB.scan_rst_n))
          end
        `endif
...
`ifdef BEHAVIORAL                         
    for (int i=0; i < 100; i++) begin
        ...
        `nnc_error("ATM14", $sformatf("`CLK_CTRL_TOP.ext_hfclk : %b is not as expectation of EXT_CLK: %b",`CLK_CTRL_TOP.ext_hfclk, rand_bit))
        end
...
`endif
    ...
    for (int i=0; i < 100; i++) begin
        ...
        `nnc_error("ATM14", $sformatf("`DIG_TOP.IOBUF_PAD[9] : %b is not as expectation of `DIG_TOP.u_pinmux.i_otp_vpp_en: %b",`SOC_TOP.IOBUF_PAD[9], rand_bit))
        end
