...
//--------------------------------------------------------------------------------------
// File Name	: soc_imeas_hpf_filter_stopband_freq_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_imeas_hpf_filter_stopband_freq_test                                             
...
// NOTES:
// This test checks the High pass filter attenuation for Fc=0.05Hz -> which should be 40db

`define TESTNAME soc_imeas_hpf_filter_stopband_freq_test
`define TESTCFG soc_imeas_hpf_filter_stopband_freq_test_cfg
...
class `TESTCFG extends soc_imeas_hpf_filter_base_test_cfg;
  ...
  function new (string name = "soc_imeas_hpf_filter_stopband_freq_test_cfg");
    super.new(name);
  endfunction: new
  ...
  function void post_randomize();
      ...
      endcase
      //imeas_sin_expected_freq = 0.5 * 1000 ;
...
class `TESTNAME extends soc_imeas_hpf_filter_base_test;
  ...
  virtual task pre_reset_phase(nnc_phase phase);
    ...
    `DUT_IF.imeas_sin_no_clk_per_period = (`DUT_IF.imeas_adc_freq*1000000)/(`DUT_IF.imeas_sin_expected_freq);
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_imeas_hpf_filter_stopband_freq_test start", UVM_LOW)

    super.main_phase(phase);

    `nnc_info("SOC_TEST", "soc_imeas_hpf_filter_stopband_freq_test end now", UVM_LOW)
    // ----------------------------------------------------------------------------------
    // End of adding test 
    // ----------------------------------------------------------------------------------
