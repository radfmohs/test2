...
//--------------------------------------------------------------------------------------
// File Name	: soc_wavegen_drva_continue_interrupt_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_wavegen_drva_continue_interrupt_test                                             
...
//***************************************************************************************
// NOTE : This test intention is to test the continue mode feature. This test constraint 
//        spi frequency such that second address interrupt will be generated.
//***************************************************************************************

// =================================================
// Testcase name is defined
// -------------------------------------------------
`define TESTNAME soc_wavegen_drva_continue_interrupt_test
`define TESTCFG soc_wavegen_drva_continue_interrupt_test_cfg
...
class `TESTCFG extends soc_wavegen_base_test_cfg;
  ...
  function new (string name = "soc_wavegen_drva_continue_interrupt_test_cfg");
    super.new(name);
...
class `TESTNAME extends soc_wavegen_base_test;
  ...
  virtual task pre_reset_phase(nnc_phase phase);
    ...
    (pclk_sel == 0)  -> hlf_wave_per inside {[100:200]};
    (pclk_sel == 1)  -> hlf_wave_per inside {[100:300]};
    (pclk_sel == 2)  -> hlf_wave_per inside {[200:300]};
    (pclk_sel == 3)  -> hlf_wave_per inside {[300:400]};
    });
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_wavegen_drva_continue_interrupt_test start", NNC_LOW)
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_INT_REG01 + (`WAVEGEN_1_ADDR_BASE * `DUT_IF.wg_drv_sel)); no_of_bytes == 3; wr_data[0] == 8'h7f; wr_data[1] == 8'h40; wr_data[2] == 8'h01;});
    ...
    // --------------------------------------------------------
    #1ms;
    `nnc_info("SOC_TEST", "soc_wavegen_drva_continue_interrupt_test end now", NNC_LOW)
  ...
  begin
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_CONFIG_REG0 + WG_BASE); wr_data[0] == {top_test_cfg.pos_dis, 1'b1, 1'b1, 1'b0, 1'b1, 1'b0, top_test_cfg.neg_ena, 1'b0};});
...
`else
    repeat(10) begin
`endif
	if(`DUT_IF.intr_length_slct_level_or_pulse === 0) begin
	  if(`DUT_IF.int_active_level_high_or_low === 1)//active high
		...
		wait(top_test_cfg.INTB_pulse_flag === 1);
		top_test_cfg.INTB_pulse_flag = 0;
	end
	...
	//for (int i=0; i<1; i++) begin //only 0th Driver A is working in this test case
	  ...
	  if (drv_int_sts[`DUT_IF.wg_drv_sel] !== 0) begin
		if(drv_int_sts[`DUT_IF.wg_drv_sel][1] === 1) begin//Second address interrupt expected in this testcase
		    `nnc_info("SOC_TEST", $sformatf("Interrupt is in block no: %d", `DUT_IF.wg_drv_sel), NNC_LOW)
		    `nnc_info("SOC_TEST", $sformatf("Interrupt WARNING: Second address (underflow) interrupt has happenned at address: %h", top_test_cfg.rd_data[0]), NNC_LOW)
		    if(`DUT_IF.clear_intr_manual_or_auto === 0) begin//if manual with local register access
		      // ------------------------------------------------------------------
		...
		if(drv_int_sts[`DUT_IF.wg_drv_sel][0] === 1) begin
		    ...
		    if (top_test_cfg.rd_data[1] === 8'h40) begin //first address is at the second half of the wave; now load the first half
			// --------------------------------------------------------
    			// Write burst starting from ADDR_WG_DRV_INT_REG02 (set next address)
    			// --------------------------------------------------------
    			assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_INT_REG02 + (`WAVEGEN_1_ADDR_BASE * `DUT_IF.wg_drv_sel)); no_of_bytes == 2; wr_data[0] == 8'h3f; wr_data[1] == 8'h00;});
			...
			if(`DUT_IF.clear_intr_manual_or_auto === 0) begin//if manual with local register access
			  ...
			  fork
			  begin
    			  assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_INT_REG01 + (`WAVEGEN_1_ADDR_BASE * `DUT_IF.wg_drv_sel)); wr_data[0] == 8'h07;});//clear both 1st & 2nd address interrupt
			...
			// --------------------------------------------------------
    			// Write burst starting from ADDR_WG_DRV_INT_REG02 (set next address)
    			// --------------------------------------------------------
    			assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_INT_REG02 + (`WAVEGEN_1_ADDR_BASE * `DUT_IF.wg_drv_sel)); no_of_bytes == 2; wr_data[0] == 8'h7f; wr_data[1] == 8'h40;});
    			`nnc_info("SOC_TEST", "Configure interrupt register", NNC_LOW)//interrupt register set as first address 64; second address 127
    			`WR_BURST_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.wr_data);
			if(`DUT_IF.clear_intr_manual_or_auto === 0) begin//if manual with local register access
			  // --------------------------------------------------------
  ...
  begin
	...
	top_test_cfg.INTB_pulse_flag = 1;
	@(posedge `DUT_IF.sys_clk);
	@(negedge `DUT_IF.sys_clk);
`ifdef BEHAVIORAL
        if((`SOC_TB.INTB !== 0) && (!(`WG_DRIVER_TOP.wg_driver_top_inst.wg_driver_int_sts[`DUT_IF.wg_drv_sel][0] === 1 && `WG_DRIVER_TOP.wg_driver_top_inst.wg_driver_int_sts[`DUT_IF.wg_drv_sel][1] === 1)))
`else
        if((`SOC_TB.INTB !== 0) && (!(`WG_DRIVER_TOP.wg_driver_top_inst.wg_driver_int_sts[`DUT_IF.wg_drv_sel*2+0] === 1 && `WG_DRIVER_TOP.wg_driver_top_inst.wg_driver_int_sts[`DUT_IF.wg_drv_sel*2+1] === 1)))
`endif
  ...
  begin
	...
	@(negedge `DUT_IF.sys_clk);
`ifdef BEHAVIORAL
        if((`SOC_TB.INTB !== 1) && (!(`WG_DRIVER_TOP.wg_driver_top_inst.wg_driver_int_sts[`DUT_IF.wg_drv_sel][0] === 1 && `WG_DRIVER_TOP.wg_driver_top_inst.wg_driver_int_sts[`DUT_IF.wg_drv_sel][1] === 1)))
`else
        if((`SOC_TB.INTB !== 1) && (!(`WG_DRIVER_TOP.wg_driver_top_inst.wg_driver_int_sts[`DUT_IF.wg_drv_sel*2+0] === 1 && `WG_DRIVER_TOP.wg_driver_top_inst.wg_driver_int_sts[`DUT_IF.wg_drv_sel*2+1] === 1)))
`endif
    	  `nnc_error("SOC_TEST", "Error! pulse INTB more than 1 pclk!")
	else
	  `nnc_info("SOC_TEST", "pulse INTB is 1 pclk!", NNC_LOW)
