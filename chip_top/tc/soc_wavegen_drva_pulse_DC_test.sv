...
//--------------------------------------------------------------------------------------
// File Name	: soc_wavegen_drva_pulse_DC_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_wavegen_drva_pulse_DC_test                                             
...
//***************************************************************************************
// NOTE : This test follows the same configuration sequence as pulse test, but 
//        the intention of this test is to generate positive/negative DC waveform
//***************************************************************************************

// =================================================
// Testcase name is defined
// -------------------------------------------------
`define TESTNAME soc_wavegen_drva_pulse_DC_test
`define TESTCFG soc_wavegen_drva_pulse_DC_test_cfg
...
class `TESTCFG extends soc_wavegen_base_test_cfg;
  ...
  rand logic [1:0] pos_neg_DC;//0: generate pos_DC; 1: generate neg_DC; 2: 0 DC (No wave)
  // -----------------------------------------------
  // End of decalration of new variables 
  // ===============================================

  function new (string name = "soc_wavegen_drva_pulse_DC_test_cfg");
    super.new(name);
  ...
  //pos_neg_DC
  constraint c_pos_neg_DC      { pos_neg_DC inside {[0:2]};}

  //silent_en
  constraint c_silent_en       { (pos_neg_DC != 2) -> silent_en == 1'b0;}

  //rest_en
  constraint c_rest_en         { (pos_neg_DC == 0) -> rest_en == 1'b0;}

  //neg_ena
  constraint c_neg_ena         { (pos_neg_DC != 1) -> neg_ena == 1'b0; (pos_neg_DC == 1) -> neg_ena == 1'b1;}

  //pos_dis
  constraint c_pos_dis         { (pos_neg_DC == 0) -> pos_dis == 1'b0; (pos_neg_DC != 0) -> pos_dis == 1'b1;}

  //points_sel
  constraint c_points_sel      { points_sel inside {0,7};}

  //waveform_sel
  constraint c_waveform_sel    { waveform_sel inside {[0:0]};}

  //load_points_sel
  constraint c_load_points_sel { (preload_sel == 2'b01) -> load_points_sel == 1'b0;}
...
class `TESTNAME extends soc_wavegen_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_wavegen_drva_pulse_DC_test start", NNC_LOW)
    ...
    wavegen_calc_clock_num(top_test_cfg.clk_freq, 100, 1000, top_test_cfg.half_period, top_test_cfg.half_period);
    ...
    // --------------------------------------------------------
    #50ms;
    `nnc_info("SOC_TEST", "soc_wavegen_drva_pulse_DC_test end now", NNC_LOW)
  ...
  begin
    ...
    `nnc_info("SOC_TEST", "Set rest period", NNC_LOW)
    `WR_BURST_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.wr_data);
    ...
    `nnc_info("SOC_TEST", "Set silent period", NNC_LOW)
    `WR_BURST_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.no_of_bytes, top_test_cfg.pads, top_test_cfg.wr_data);
    ...
    // --------------------------------------------------------
    assert(top_test_cfg.randomize() with {reg_addr == (`SOC_ADDR_WG_DRV_CONFIG_REG0 + WG_BASE); wr_data[0] == {top_test_cfg.pos_dis, 1'b1, 1'b0, 1'b0, 1'b1, top_test_cfg.silent_en, top_test_cfg.neg_ena, top_test_cfg.rest_en};});
    `nnc_info("SOC_TEST", "Set driver configuration register", NNC_LOW)
    `WR_WAVEGEN_REG(top_test_cfg.reg_addr, top_test_cfg.wr_data[0], top_test_cfg.pads);
    if(top_test_cfg.pos_neg_DC === 1)
	`nnc_info("SOC_TEST", "Generate NEG_DC", NNC_LOW)
    else if(top_test_cfg.pos_neg_DC === 0)
	`nnc_info("SOC_TEST", "Generate POS_DC", NNC_LOW)
    else
	`nnc_info("SOC_TEST", "Generate 0 DC (No Wave)", NNC_LOW)
