...
// Description	: Testcase soc_zmeas_stim_mon_leadoff_auto_pair_loop_test                                             
// Designer	: shreeyal@nanochap.com                                                                 
// Date		: 27-05-2025                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_zmeas_stim_mon_leadoff_auto_pair_loop_test
`define TESTCFG soc_zmeas_stim_mon_leadoff_auto_pair_loop_test_cfg
...
class `TESTCFG extends soc_zmeas_stim_mon_auto_pair_loop_test_cfg;
  ...
  function new (string name = "soc_zmeas_stim_mon_leadoff_auto_pair_loop_test_cfg");
    super.new(name);
  endfunction: new
  ...
  constraint c_leadoff_int_en              { leadoff_int_en inside {[1:1]};}  

  constraint c_leadoff_int_to_pin_en       { leadoff_int_to_pin_en inside {[1:1]};}  

  //constraint c_leadoff_th                  { leadoff_th inside {[1:511]};}  // whole range is 1023 , leadoff happens in half of whole range - from 512 to 1023 - so leadoff_th max is 511
  constraint c_leadoff_th                  { leadoff_th inside {[1:200]};}  // whole range is 1023 , leadoff happens in half of whole range - from 512 to 1023 - so leadoff_th max is 511
  //constraint c_leadoff_th                  { leadoff_th == 200 ;}  
...
class `TESTNAME extends soc_zmeas_stim_mon_auto_pair_loop_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    //for(int i =0; i< 3; i++)begin
      wait_for_intb();
      check_int_sts_reg("LEADOFF", 1); 
      clear_int_sts_reg("LEADOFF");
      wait_for_intb_clear();
    end
    ...
    // 2.change the leadoff parameters
    `DUT_IF.leadoff_th            = top_test_cfg.leadoff_th;
    `DUT_IF.leadoff_short_th_tgt  = top_test_cfg.leadoff_short_th_tgt;
    ...
    `nnc_info("SOC_TEST", "soc_zmeas_stim_mon_leadoff_auto_pair_loop_test end now", NNC_LOW)
    // ----------------------------------------------------------------------------------
    // End of adding test 
    // ----------------------------------------------------------------------------------
  ...
  task leadoff_intr_checks();
    ...
    `DUT_IF.leadoff_int_en = 0;
    `DUT_IF.leadoff_int_to_pin_en = 1;
    top_test_cfg.wr_data[0] = {4'b0,`DUT_IF.short_int_to_pin_en,`DUT_IF.leadoff_int_to_pin_en,`DUT_IF.short_int_en,`DUT_IF.leadoff_int_en};
    `WR_NORMAL_REG(`SOC_STIM_MON_LOFF_SHORT_INT_CTRL, top_test_cfg.wr_data[0], top_test_cfg.pads);
